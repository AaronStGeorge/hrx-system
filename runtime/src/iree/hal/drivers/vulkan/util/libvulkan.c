...
#define IREE_HAL_VULKAN_DEFINE_DEVICE_VkDeviceAddress(result_type, symbol, \
                                                      decl, args)          \
  IREE_API_EXPORT result_type iree_##symbol##_raw(                         \
      const iree_hal_vulkan_device_syms_t* IREE_RESTRICT syms _COMMA_DECL( \
          decl)) {                                                         \
  ...
  IREE_API_EXPORT result_type iree_##symbol(                               \
      const iree_hal_vulkan_device_syms_t* IREE_RESTRICT syms,             \
      const char* file, uint32_t line _COMMA_DECL(decl)) {                 \
    (void)file;                                                            \
    (void)line;                                                            \
    return iree_##symbol##_raw(syms _COMMA_ARGS(args));                    \
  }
...
#undef IREE_HAL_VULKAN_DEFINE_DEVICE_VkDeviceAddress
