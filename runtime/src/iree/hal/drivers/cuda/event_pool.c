...
struct iree_hal_cuda_event_t {
  ...
  iree_hal_cuda_event_pool_t* pool;
  // The underlying CUevent object.
  CUevent cu_event;
};

CUevent iree_hal_cuda_event_handle(const iree_hal_cuda_event_t* event) {
  return event->cu_event;
}

static inline void iree_hal_cuda_event_destroy(iree_hal_cuda_event_t* event) {
  iree_allocator_t host_allocator = event->host_allocator;
  const iree_hal_cuda_dynamic_symbols_t* symbols = event->symbols;
...
static inline iree_status_t iree_hal_cuda_event_create(
    const iree_hal_cuda_dynamic_symbols_t* symbols,
    iree_hal_cuda_event_pool_t* pool, iree_allocator_t host_allocator,
...
static void iree_hal_cuda_event_pool_release_event(
    iree_hal_cuda_event_pool_t* event_pool, iree_host_size_t event_count,
...
void iree_hal_cuda_event_release(iree_hal_cuda_event_t* event) {
  ...
  if (iree_atomic_ref_count_dec(&event->ref_count) == 1) {
    ...
    iree_hal_cuda_event_pool_release_event(pool, 1, &event);
    // Drop our reference to the pool itself when we return event to it.
    iree_hal_cuda_event_pool_release(pool);  // -1
  }
}
...
struct iree_hal_cuda_event_pool_t {
  ...
  iree_hal_cuda_event_t* available_list[] IREE_GUARDED_BY(event_mutex);
};
...
iree_status_t iree_hal_cuda_event_pool_allocate(
  ...
  *out_event_pool = NULL;
  ...
  iree_hal_cuda_event_pool_t* event_pool = NULL;
  ...
  for (iree_host_size_t i = 0; i < available_capacity; ++i) {
    status = iree_hal_cuda_event_create(
...
iree_status_t iree_hal_cuda_event_pool_acquire(
    iree_hal_cuda_event_pool_t* event_pool, iree_host_size_t event_count,
    iree_hal_cuda_event_t** out_events) {
  ...
  // new iree_hal_cuda_event_t objects.
  ...
  if (remaining_count > 0) {
    ...
    for (iree_host_size_t i = 0; i < remaining_count; ++i) {
      status = iree_hal_cuda_event_create(event_pool->symbols, event_pool,
                                          event_pool->host_allocator,
                                          &out_events[from_pool_count + i]);
      if (!iree_status_is_ok(status)) {
        // Must release all events we've acquired so far.
        iree_hal_cuda_event_pool_release_event(event_pool, from_pool_count + i,
                                               out_events);
  ...
  for (iree_host_size_t i = 0; i < event_count; ++i) {
    iree_hal_cuda_event_pool_retain(out_events[i]->pool);  // +1
  }
...
static void iree_hal_cuda_event_pool_release_event(
  ...
  if (to_pool_count > 0) {
    for (iree_host_size_t i = 0; i < to_pool_count; ++i) {
      ...
      iree_hal_cuda_event_retain(events[i]);  // -> 1
    }
  ...
  if (remaining_count > 0) {
    ...
    for (iree_host_size_t i = 0; i < remaining_count; ++i) {
      iree_hal_cuda_event_destroy(events[to_pool_count + i]);
    }
    IREE_TRACE_ZONE_END(z1);
  }
  IREE_TRACE_ZONE_END(z0);
