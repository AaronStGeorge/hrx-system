...
static const char kIreeRunReplayUsage[] =
    ...
    "while\n"
    "device work is ordered by the captured semaphore, event, command-buffer,\n"
    "and barrier dependencies. Select the target topology with --device= just\n"
    "like iree-run-module.\n"
    "\n"
    "Usage:\n"
    ...
    "      every captured executable. Replay validates export counts and "
    "reflected\n"
    "      ABI shape before using replacements.\n"
    "  --device_profiling_mode=...\n"
    "  --device_profiling_output=path.ireeprof\n"
    "      Captures HAL-native profiling for the replay execution.\n"
    "  --agents_md\n"
    "      Prints a Markdown playbook suitable for AGENTS.md.\n"
    "\n"
    "Examples:\n"
...
static void iree_run_replay_print_agent_markdown(FILE* file) {
  fputs(
      "# IREE HAL Replay\n"
      "\n"
      "Use HAL replay when you need a reproducible HAL-level workload from an\n"
      "IREE application. A `.ireereplay` capture records the HAL resource "
      "graph,\n"
      "host-visible byte payloads, executable payloads, external file "
      "references,\n"
      "command buffers, queue operations, and synchronization edges.\n"
      "\n"
      "Replay is separate from profiling. `.ireereplay` says what HAL work to\n"
      "run; `.ireeprof` says what happened while that run executed. A normal\n"
      "workflow captures once, replays many times, and optionally captures\n"
      "`.ireeprof` bundles from `iree-benchmark-replay`.\n"
      "\n"
      "## Capture\n"
      "\n"
      "`iree-run-module` and `iree-benchmark-module` can record HAL work with\n"
      "`--device_replay_output=/tmp/model.ireereplay`. Keep the normal "
      "module,\n"
      "function, parameter, input, and device flags unchanged.\n"
      "\n"
      "```bash\n"
      ...
      "archives. FD-backed HAL files are recorded by path and validation "
      "metadata\n"
      "instead of copied into every capture. Use `capture-ranges` for "
      "hermetic\n"
      "correctness replays that only need file bytes observed through HAL "
      "reads.\n"
      "Use `capture-all` only for small external files. Use `fail` when "
      "fd-backed\n"
      "external files must be rejected.\n"
      "\n"
      "The default validation mode is `identity`, which records length, "
      "device,\n"
      "inode, and modification time without scanning file contents. Use "
      "`digest`\n"
      "only when referenced files will be copied or staged to another "
      "filesystem\n"
      "and a full byte scan during capture and replay is acceptable.\n"
      "\n"
      "`iree-run-module` emits standard replay scopes named `init`, "
      "`execute`,\n"
      "and `deinit`. `iree-benchmark-module` emits `execute` scopes around "
      "the\n"
      "same timed region reported by the benchmark.\n"
      "\n"
      "## Run\n"
      "\n"
      "Run a replay once with the same target device shape you want to test:\n"
      "\n"
      "```bash\n"
      "iree-run-replay --device=local-sync /tmp/model.ireereplay\n"
      "```\n"
      "\n"
      "If the capture referenced external files from another mount root, "
      "remap\n"
      "the captured prefix before replay opens files:\n"
      "\n"
      "```bash\n"
      "iree-run-replay --device=local-sync \\\n"
      ...
      "```\n"
      "\n"
      "The remapped file must still match recorded identity or digest "
      "metadata.\n"
      "A mismatch is a hard failure because otherwise the replay could "
      "silently\n"
      "benchmark a different model or dataset.\n"
      "\n"
      "## Substitute Executables\n"
      "\n"
      "Use `iree-dump-replay` to find executable object ids, then substitute "
      "a\n"
      "replacement payload while preserving the captured HAL workload:\n"
      "\n"
      "```bash\n"
      "iree-dump-replay --format=text /tmp/model.ireereplay | grep executable\n"
      ...
      "counts,\n"
      "constant counts, binding counts, workgroup sizes, and reflected "
      "parameter\n"
      "layouts when the capture has metadata.\n"
      "\n"
      "## Benchmark And Profile\n"
      "\n"
      "`iree-benchmark-replay` runs the same replay under Google Benchmark:\n"
      "\n"
      "```bash\n"
      ...
      "scope markers while still executing the complete capture each "
      "iteration.\n"
      "See `iree-benchmark-replay --help` or `iree-benchmark-replay "
      "--agents_md`\n"
      "for benchmark-only behavior.\n"
      "\n"
      "Capture HAL profiling from replayed benchmark iterations with normal "
      "device\n"
      "profiling flags:\n"
      "\n"
      "```bash\n"
      ...
      "## Inspect\n"
      "\n"
      "`iree-dump-replay --format=text` is for quick inspection. "
      "`--format=jsonl`\n"
      "is for jq and agent workflows:\n"
      "\n"
      "```bash\n"
      ...
      "See `iree-dump-replay --help` or `iree-dump-replay --agents_md` for\n"
      "dump-only details.\n"
      "\n"
      "## Failure Contract\n"
      "\n"
      "Replay must fail loudly when it cannot reproduce captured HAL work. "
      "Missing\n"
      ...
      "\n"
      "When debugging a capture, keep the `.ireereplay` and referenced files\n"
      "together, dump JSONL first, run once with `iree-run-replay`, then "
      "benchmark\n"
      "with profiling only after the single-run path is clean.\n",
      file);
}
...
int main(int argc, char** argv) {
  ...
  iree_flags_set_usage("iree-run-replay", kIreeRunReplayUsage);
  iree_flags_parse_checked(IREE_FLAGS_PARSE_MODE_DEFAULT, &argc, &argv);
  if (FLAG_agents_md) {
    iree_run_replay_print_agent_markdown(stdout);
    fflush(stdout);
