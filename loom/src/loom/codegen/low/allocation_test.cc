...
TEST(LowAllocationAssignmentRangeTest, MatchesPhysicalRegisterClass) {
  ...
  EXPECT_FALSE(loom_low_allocation_assignment_is_physical_register_class(
      &assignment, 4));
  EXPECT_FALSE(
      loom_low_allocation_assignment_is_physical_register_class(NULL, 3));

  const loom_low_allocation_assignment_t target_id_assignment = {
      .descriptor_reg_class_id = 3,
  ...
  const loom_low_allocation_assignment_t empty_assignment = {
      .descriptor_reg_class_id = 3,
      .location_kind = LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER,
  };
  EXPECT_FALSE(loom_low_allocation_assignment_is_physical_register_class(
...
TEST(LowAllocationAssignmentRangeTest, ComputesLocationEnd) {
  ...
  EXPECT_FALSE(
      loom_low_allocation_assignment_location_exclusive_end(NULL, &end));
  EXPECT_EQ(end, 0u);
  EXPECT_FALSE(
      loom_low_allocation_assignment_location_exclusive_end(&assignment, NULL));
...
TEST(LowAllocationAssignmentRangeTest, MatchesAndOverlapsConcreteRanges) {
  ...
  const loom_low_allocation_assignment_t same = {
      .descriptor_reg_class_id = 1,
      .location_kind = LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER,
      .location_base = 4,
      .location_count = 2,
  };
  const loom_low_allocation_assignment_t overlapping = {
      .descriptor_reg_class_id = 1,
      .location_kind = LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER,
      .location_base = 5,
      .location_count = 2,
  };
  const loom_low_allocation_assignment_t adjacent = {
      .descriptor_reg_class_id = 1,
      .location_kind = LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER,
      .location_base = 6,
      .location_count = 2,
  };
  const loom_low_allocation_assignment_t different_class = {
  ...
  EXPECT_TRUE(loom_low_allocation_assignments_match(&lhs, &same));
  EXPECT_TRUE(loom_low_allocation_assignments_overlap(&lhs, &same));
  EXPECT_FALSE(loom_low_allocation_assignments_match(&lhs, &overlapping));
  EXPECT_TRUE(loom_low_allocation_assignments_overlap(&lhs, &overlapping));
  EXPECT_FALSE(loom_low_allocation_assignments_overlap(&lhs, &adjacent));
  EXPECT_FALSE(loom_low_allocation_assignments_overlap(&lhs, &different_class));

  const loom_low_allocation_assignment_t empty = {
      .descriptor_reg_class_id = 1,
      .location_kind = LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER,
      .location_base = 4,
  };
  EXPECT_FALSE(loom_low_allocation_assignments_match(&lhs, &empty));
  EXPECT_FALSE(loom_low_allocation_assignments_overlap(&lhs, &empty));
}

TEST(LowAllocationAssignmentRangeTest, MatchesAndOverlapsTargetAliasRanges) {
  ...
  const loom_low_allocation_assignment_t alias_same = {
      .descriptor_reg_class_id = 1,
      .location_kind = LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER,
      .location_base = 4,
      .location_count = 2,
  };
  const loom_low_allocation_assignment_t alias_overlap = {
      .descriptor_reg_class_id = 1,
      .location_kind = LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER,
      .location_base = 5,
      .location_count = 2,
  ...
  EXPECT_FALSE(loom_low_allocation_assignments_match(&lhs, &alias_same));
  EXPECT_TRUE(loom_low_allocation_assignments_match_target_storage(
      &descriptor_set, &lhs, &alias_same));
  EXPECT_TRUE(loom_low_allocation_assignments_overlap_target_storage(
