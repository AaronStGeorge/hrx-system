...
static iree_status_t loom_run_execution_provider_validate(
...
static iree_status_t loom_run_execution_environment_check_unique_provider_name(
...
static iree_status_t loom_run_execution_environment_check_unique_backend_names(
    const loom_run_execution_environment_t* environment) {
  for (iree_host_size_t i = 0; i < environment->hal_backend_count; ++i) {
    const loom_run_hal_backend_t* backend = environment->hal_backends[i];
    for (iree_host_size_t j = 0; j < i; ++j) {
      const loom_run_hal_backend_t* existing = environment->hal_backends[j];
...
iree_status_t loom_run_execution_environment_initialize(
  ...
  *out_environment = (loom_run_execution_environment_t){
  ...
  for (iree_host_size_t i = 0; i < provider_set->provider_count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(loom_run_execution_provider_validate(provider, i));
    IREE_RETURN_IF_ERROR(
        loom_run_execution_environment_check_unique_provider_name(provider_set,
                                                                  i));
