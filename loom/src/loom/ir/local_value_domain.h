...
//
// A local value domain installs a dense value_id -> value_ordinal map into the
...
enum loom_local_value_domain_flag_bits_e {
  ...
  LOOM_LOCAL_VALUE_DOMAIN_FLAG_INSTALLED = 1u << 0,
};
...
// Installs a local value domain for |region| into |module|'s ordinal scratch.
//
// The module and region must stay semantically immutable until deinitialize.
// The domain registers block arguments, op results, op operands, SSA
...
// Clears all installed value IDs and releases the module ordinal scratch map.
...
static inline loom_value_ordinal_t loom_local_value_domain_ordinal(
  ...
  IREE_ASSERT_ARGUMENT(domain);
  IREE_ASSERT(
      iree_any_bit_set(domain->flags, LOOM_LOCAL_VALUE_DOMAIN_FLAG_INSTALLED));
