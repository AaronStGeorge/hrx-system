...
static uint32_t loom_cfg_saturating_u32(iree_host_size_t value) {
  if (value > UINT32_MAX) return UINT32_MAX;
  return (uint32_t)value;
}

static bool loom_cfg_value_id_is_defined(const loom_module_t* module,
                                         loom_value_id_t value_id) {
  return value_id != LOOM_VALUE_ID_INVALID && value_id < module->values.count;
}

static iree_status_t loom_cfg_emit_successor_arg_count_mismatch(
  ...
  loom_diagnostic_param_t params[] = {
      ...
      loom_param_u32(loom_cfg_saturating_u32(actual_count)),
      loom_param_u32(loom_cfg_saturating_u32(expected_count)),
  };
  return loom_cfg_emit(emitter, op,
                       loom_error_def_lookup(LOOM_ERROR_DOMAIN_STRUCTURE, 25),
                       params, IREE_ARRAYSIZE(params));
}

static iree_status_t loom_cfg_emit_successor_arg_type_mismatch(
    iree_diagnostic_emitter_t emitter, const loom_op_t* op,
    iree_string_view_t op_name, uint8_t successor_index,
...
static iree_status_t loom_cfg_verify_successor_args(
  ...
  for (uint16_t i = 0; i < target->arg_count; ++i) {
    ...
    if (!loom_cfg_value_id_is_defined(module, actual_id) ||
        !loom_cfg_value_id_is_defined(module, expected_id)) {
      continue;
    }
...
iree_status_t loom_cfg_br_verify(const loom_module_t* module,
  ...
  return loom_cfg_verify_successor_args(module, emitter, op, IREE_SV("cfg.br"),
...
iree_status_t loom_cfg_cond_br_verify(const loom_module_t* module,
                                      const loom_op_t* op,
                                      iree_diagnostic_emitter_t emitter) {
  IREE_RETURN_IF_ERROR(loom_cfg_verify_successor_args(
      module, emitter, op, IREE_SV("cfg.cond_br"), 0,
      loom_cfg_cond_br_true_dest(op), NULL, 0));
  return loom_cfg_verify_successor_args(
