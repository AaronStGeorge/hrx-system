...
TEST_F(LowVerifyTest, StorageTrafficPassesWithOwnedStorage) {
  ...
  loom_verify_result_t result = VerifySource(
      ...
      ": low.storage<scratch>\n"
      "  %view = low.storage.view %storage {byte_length = 4} : "
      "low.storage<scratch> -> low.storage<scratch>\n"
      "  low.spill %input, %view {offset = 0} : reg<vm.i32>, "
      "low.storage<scratch>\n"
      "  %reload = low.reload %view {offset = 0} : low.storage<scratch> "
      "-> reg<vm.i32>\n"
      "  %addr = low.storage.address %view : "
