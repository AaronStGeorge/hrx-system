...
typedef struct loom_vector_memory_footprint_state_t {
  // Current pass instance used for statistics and scratch arena ownership.
  loom_pass_t* pass;

  // Module whose function body is being checked.
  loom_module_t* module;

  // Per-function value facts computed before footprint checks.
...
static void loom_vector_memory_footprint_add_stat(
    loom_vector_memory_footprint_state_t* state, uint16_t statistic_index,
    int64_t delta) {
...
static iree_string_view_t loom_vector_memory_footprint_origin_name(
  ...
  for (uint8_t i = 0; i <= axis; ++i) {
    ...
    if (i == axis) {
      if (dynamic_ordinal >= dynamic_indices.count) return IREE_SV("<unknown>");
...
static iree_status_t loom_vector_memory_footprint_fail_axis(
  ...
  IREE_RETURN_IF_ERROR(loom_vector_memory_footprint_emit_failure(
      state, access, view_axis, vector_axis, origin, reason, required));
  return iree_make_status(
      IREE_STATUS_FAILED_PRECONDITION,
      "%.*s footprint is not proven in-bounds on view axis %u: %.*s",
      (int)op_name.size, op_name.data, (unsigned)view_axis, (int)reason.size,
      reason.data);
}
...
static iree_status_t loom_vector_memory_footprint_fail_general(
  ...
  return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
                          "%.*s footprint is not proven in-bounds: %.*s",
                          (int)op_name.size, op_name.data, (int)reason.size,
                          reason.data);
}
...
static iree_status_t loom_vector_memory_footprint_fail_linear_span(
  ...
  return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
                          "%.*s linear footprint is not proven in-bounds: %.*s",
                          (int)op_name.size, op_name.data, (int)reason.size,
                          reason.data);
}
...
static iree_status_t loom_vector_memory_footprint_origin_expr(
  ...
  *out_known = false;
  ...
  for (uint8_t i = 0; i <= axis; ++i) {
    ...
    if (i == axis) {
      if (dynamic_ordinal >= dynamic_indices.count) return iree_ok_status();
...
static bool loom_vector_memory_footprint_mask_uniform_false(
  ...
  if (mask == LOOM_VALUE_ID_INVALID) return false;
...
static iree_status_t loom_vector_memory_footprint_tail_end_expr(
  ...
  *out_tail = false;
  ...
  if (!lower_matches_origin) return iree_ok_status();
...
static bool loom_vector_memory_footprint_offset_bounds_from_facts(
  ...
  for (iree_host_size_t i = 0; i < lanes.count; ++i) {
    ...
    if (lane_lower < lower) lower = lane_lower;
    if (lane_upper > upper) upper = lane_upper;
  }
  *out_lower = lower;
  *out_upper = upper;
  return true;
...
static bool loom_vector_memory_footprint_value_defines_iota(
  ...
  *out_op = NULL;
  ...
  if (loom_value_is_block_arg(value)) return false;
  const loom_op_t* op = loom_value_def_op(value);
  if (!op || !loom_vector_iota_isa(op)) return false;
  *out_op = op;
  return true;
}
...
static iree_status_t loom_vector_memory_footprint_iota_offset_bounds(
  ...
  *out_known = false;
  if (loom_type_rank(offsets_type) != 1) return iree_ok_status();
...
static iree_status_t loom_vector_memory_footprint_offset_bounds(
  ...
  *out_known = false;
  ...
  if (loom_vector_memory_footprint_value_defines_iota(
    ...
    if (*out_known) return iree_ok_status();
  }
...
static iree_status_t loom_vector_memory_footprint_check_access(
  ...
  if (loom_vector_memory_footprint_access_empty(access) ||
    ...
    loom_vector_memory_footprint_add_stat(
        state, LOOM_VECTOR_MEMORY_FOOTPRINT_STAT_OPS_SKIPPED, 1);
    return iree_ok_status();
  }

  loom_vector_memory_footprint_add_stat(
      state, LOOM_VECTOR_MEMORY_FOOTPRINT_STAT_OPS_CHECKED, 1);
