...
static iree_status_t loom_check_production_initialize_low_descriptor_registry(
    void* user_data, loom_target_low_descriptor_registry_t* out_registry) {
  IREE_ASSERT_ARGUMENT(out_registry);
  const loom_check_production_runner_t* runner =
      (const loom_check_production_runner_t*)user_data;
  if (!runner || !runner->initialize_low_descriptor_registry) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "production loom-check runner has no low descriptor registry");
  }
  runner->initialize_low_descriptor_registry(out_registry);
  return iree_ok_status();
}

static iree_status_t loom_check_production_initialize_low_lower_policy_registry(
    void* user_data, loom_low_lower_policy_registry_t* out_registry) {
  IREE_ASSERT_ARGUMENT(out_registry);
  const loom_check_production_runner_t* runner =
      (const loom_check_production_runner_t*)user_data;
  if (!runner || !runner->initialize_low_lower_policy_registry) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "production loom-check runner has no source-to-low policy registry");
  }
  runner->initialize_low_lower_policy_registry(out_registry);
  return iree_ok_status();
}
...
int loom_check_production_main(int argc, char** argv,
                               const loom_check_production_runner_t* runner) {
  if (!runner) {
    fprintf(stderr, "production loom-check runner configuration is required\n");
    return 1;
  }
  ...
  const loom_check_environment_t environment = {
      ...
      .initialize_low_descriptor_registry =
          {
              .fn = loom_check_production_initialize_low_descriptor_registry,
