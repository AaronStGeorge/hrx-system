...
static void loom_compile_print_agents_markdown(FILE* stream) {
  fprintf(
      ...
      "allocation_spill_count:.allocation.spill_count}, "
      "(.entries.rows[]? | {function, source_function, "
      "target_export_symbol, code_byte_count, allocation_spill_count})' "
      "report.json\n"
      ...
      "per-entry summary index. Detail mode keeps row arrays such as\n"
      "`pressure_rows.rows[]`, `source_low.rows[]`,\n"
      "`source_low.memory_rows[]`, `math_legalization.rows[]`, and\n"
      "`target_legalization.rows[]`; those rows carry `function`,\n"
