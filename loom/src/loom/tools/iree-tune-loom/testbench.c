...
enum {
  IREE_TUNE_LOOM_MAX_REQUIREMENT_PROVIDERS = 8,
};
...
iree_status_t iree_tune_loom_evaluate_case_requirements(
  ...
  loom_testbench_requirement_provider_t
      requirement_providers[IREE_TUNE_LOOM_MAX_REQUIREMENT_PROVIDERS] = {0};
  ...
  if (requirement_provider_count > IREE_ARRAYSIZE(requirement_providers)) {
    return iree_make_status(
        ...
        "iree-tune-loom requirement provider capacity exceeded");
  }
...
void iree_tune_loom_configure_reference_oracles(
  ...
  *out_oracles = (iree_tune_loom_reference_oracles_t){0};
  ...
  inout_execution_options->invocation.oracle_providers =
      loom_make_testbench_oracle_provider_list(
          ...
          IREE_TUNE_LOOM_REFERENCE_ORACLE_PROVIDER_COUNT);
}
