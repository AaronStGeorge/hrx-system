...
// Testbench integration helpers for iree-tune-loom.

#ifndef LOOM_TOOLS_IREE_TUNE_LOOM_TESTBENCH_H_
#define LOOM_TOOLS_IREE_TUNE_LOOM_TESTBENCH_H_
...
enum {
  ...
  IREE_TUNE_LOOM_REFERENCE_ORACLE_PROVIDER_COUNT = 2,
};

typedef struct iree_tune_loom_reference_oracles_t {
  ...
  loom_testbench_oracle_provider_t
      providers[IREE_TUNE_LOOM_REFERENCE_ORACLE_PROVIDER_COUNT];
} iree_tune_loom_reference_oracles_t;

// Evaluates check.requires/check.skip_if predicates for |case_plan|.
iree_status_t iree_tune_loom_evaluate_case_requirements(
...
void iree_tune_loom_configure_reference_oracles(
    loom_run_hal_testbench_context_t* context, iree_allocator_t host_allocator,
    iree_tune_loom_reference_oracles_t* out_oracles,
...
#endif  // LOOM_TOOLS_IREE_TUNE_LOOM_TESTBENCH_H_
