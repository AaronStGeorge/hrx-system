...
void iree_benchmark_loom_print_agents_md(FILE* file) {
  fprintf(
      ...
      "`sample_id`, `sample_index`, and a concrete `sample.parameters` map "
      "on sample/benchmark/profile rows. For dispatch benchmarks, "
      "`--sample_compilation=once` compiles once, "
      "`--sample_compilation=per_sample` compiles each selected sample "
      "with concrete parameter facts, and `--sample_compilation=both` "
      ...
      "windows for one concrete sample per candidate and emits "
      ...
      "`--sample=` when a case has multiple samples. "
      ...
      "unavailable profiles use the same status row layout instead of "
      ...
      "```\n"
      "\n"
      "Use `iree-benchmark-loom --help` for the full flag list, JSONL row "
