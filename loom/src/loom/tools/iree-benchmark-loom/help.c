...
void iree_benchmark_loom_print_agents_md(FILE* file) {
  fprintf(
      ...
      "`--output_format=jsonl` switches to the streaming lifecycle event log "
      ...
      "dispatch benchmarks, `--sample_compilation=once` compiles once, "
      "`--sample_compilation=per_sample` compiles each selected sample with "
      "concrete parameter facts, and `--sample_compilation=both` emits both "
      "result sets. `check.file.read.npy` paths resolve relative to the "
      "`.loom` file; `check.file.write.npy` paths are rooted under "
      "`--file_output_dir`, which defaults under "
      ...
      "target and `--input_ring_count=1` forces hot-reuse timing when that is "
      "the experiment. `--profile_final_batch=true` collects dispatch profile "
      "evidence outside the measured window. `--profile_artifacts_dir=DIR` "
      ...
      "`--artifact_bundle_dir=DIR` collects the result file, `manifest.json`, "
      ...
      "`results.jsonl`. `--artifact_bundle_policy=debug|full` also writes "
      ...
      "packages under `hal_executables/`. Debug JSONL compile and benchmark "
      "rows link those files from `compile_report_path`, "
      "`target_artifact_path`, `target_listing_path`, and "
      "`hal_executable_path`.\n"
      "\n"
      "Examples:\n"
      "\n"
      ...
      "--output_format=jsonl --output=ababa.jsonl\n"
      "```\n"
      "\n"
      "```bash\n"
...
void iree_benchmark_loom_set_usage(const char* tool_name) {
  iree_flags_set_usage(
      ...
      "  iree-benchmark-loom file.loom --benchmark=@smoke_latency --dry_run "
      "--output=plan.json\n"
      "  iree-benchmark-loom file.loom --device=amdgpu "
      "--benchmark=@kernel_latency --batch_size=64 "
      "--profile_final_batch=true --output=results.json\n"
      "  iree-benchmark-loom file.loom --device=amdgpu "
      "--benchmark=@kernel_latency --profile_final_batch=true "
      "--profile_artifacts_dir=/tmp/loom-profiles --output=results.json\n"
      "  iree-benchmark-loom file.loom --device=amdgpu "
      ...
      "--sample_compilation=once|per_sample|both\n"
      "  iree-benchmark-loom file.loom --device=amdgpu "
      ...
      "\n"
      "JSONL event stream (--output_format=jsonl):\n"
      ...
      "use and agent ingestion. JSONL rows are written and flushed as evidence "
      "becomes available, so an agent can `tail -f` an --output file during "
      "long sweeps. Every JSONL row carries run_id. Selected benchmark rows "
      "also carry "
      ...
      "`--sample_compilation=once` compiles once, "
      "`--sample_compilation=per_sample` compiles each selected sample with "
      "concrete parameter facts, and `--sample_compilation=both` emits both "
      ...
      "only when --profile_final_batch=true; "
      ...
      "--file_output_dir, which defaults under $TMPDIR/iree-loom-benchmark. "
      ...
      "  jq 'select(.row==\"benchmark\") | .benchmark_result | "
      "{benchmark,p50:.operation_timing_ns.p50,data_cache}' results.jsonl\n"
      "  jq 'select(.row==\"benchmark\" and .sample) | "
      "{candidate_id,sample_id,sample:.sample.parameters,"
      "p50:.benchmark_result.operation_timing_ns.p50}' results.jsonl\n"
      "  jq 'select(.row==\"benchmark.repetition\") | "
