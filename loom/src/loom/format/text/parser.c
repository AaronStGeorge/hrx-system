...
static iree_status_t loom_parse_low_asm_descriptor_set(
    loom_parser_t* parser, const void** out_descriptor_set) {
  *out_descriptor_set = NULL;
  ...
  iree_status_t status =
      parser->low_asm_environment.vtable->lookup_descriptor_set(
          parser->low_asm_environment.user_data, key_token.text,
          out_descriptor_set);
  ...
  if (*out_descriptor_set == NULL) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "low asm descriptor-set lookup returned no descriptor set");
  }
  return iree_ok_status();
}
...
static iree_status_t loom_low_asm_immediate_descriptor(
  ...
  return parser->low_asm_environment.vtable->immediate_descriptor(
      parser->low_asm_environment.user_data, packet, immediate_index,
      out_immediate);
}
...
static iree_status_t loom_parse_low_asm_return(
  ...
  if (loom_tokenizer_at(&parser->tokenizer, LOOM_TOKEN_SSA_VALUE)) {
    for (;;) {
      ...
      if (!loom_tokenizer_at(&parser->tokenizer, LOOM_TOKEN_SSA_VALUE)) break;
  ...
  if (loom_tokenizer_try_consume(&parser->tokenizer, LOOM_TOKEN_COLON)) {
    ...
    for (iree_host_size_t i = 0; i < values.count; ++i) {
      if (i > 0) LOOM_PARSE_EXPECT(parser, LOOM_TOKEN_COMMA, NULL);
  ...
  IREE_RETURN_IF_ERROR(parser->low_asm_environment.vtable->build_return(
      parser->low_asm_environment.user_data, &parser->builder, values.values,
...
static iree_status_t loom_parse_low_asm_instruction(
    loom_parser_t* parser, const void* descriptor_set,
  ...
  iree_status_t status = parser->low_asm_environment.vtable->lookup_packet(
      parser->low_asm_environment.user_data, descriptor_set,
      mnemonic_token.text, &packet);
  if (iree_status_is_not_found(status)) {
    iree_status_ignore(status);
    return loom_parser_emit_low_asm_error(parser, mnemonic_token,
  ...
  if (iree_status_is_already_exists(status)) {
    iree_status_ignore(status);
    return loom_parser_emit_low_asm_error(
        parser, mnemonic_token, IREE_SV("ambiguous low asm mnemonic"));
  }
  IREE_RETURN_IF_ERROR(status);
  ...
  IREE_RETURN_IF_ERROR(parser->low_asm_environment.vtable->build_packet(
      ...
      packet.result_count, location, &op));
  if (op == NULL) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
...
static iree_status_t loom_parse_low_asm_packet_impl(
    loom_parser_t* parser, const void* descriptor_set, loom_token_t start_token,
    const iree_string_view_t* comments, iree_host_size_t comment_count,
    loom_parsed_op_t* parsed_spans) {
  const uint32_t errors_before = parser->error_count;
...
static iree_status_t loom_parse_low_asm_packet(loom_parser_t* parser,
...
static iree_status_t loom_parse_low_asm_region_body(
    loom_parser_t* parser, const loom_region_descriptor_t* region_descriptor,
    const void* descriptor_set, loom_region_t* region,
...
static iree_status_t loom_parse_low_asm_braced_region(
  ...
  iree_host_size_t pending_successor_start =
      parser->pending_successor_refs.count;
  bool region_end_consumed = false;
  iree_status_t status = loom_parse_low_asm_region_body(
      parser, region_descriptor, descriptor_set, region, &region_end_consumed);
  if (iree_status_is_ok(status) && parser->error_count == errors_before) {
    status = loom_parser_resolve_pending_successor_refs(
        parser, region, pending_successor_start);
  }
