...
const loom_cpu_packed_dot_descriptor_t* FindDescriptor(const char* name) {
  return loom_cpu_packed_dot_find_by_name(iree_make_cstring_view(name));
}

loom_cpu_packed_dot_feature_bits_t FeatureBits(const char* name) {
  loom_cpu_packed_dot_feature_bits_t feature_bits = 0;
  IREE_EXPECT_OK(loom_cpu_packed_dot_feature_bits_for_name(
      iree_make_cstring_view(name), &feature_bits));
  return feature_bits;
}

loom_cpu_packed_dot_match_request_t MatchRequest(
    loom_cpu_packed_dot_family_t family, uint16_t vector_bit_width,
...
TEST(PackedDotContractTest, DescriptorNamesAreUnique) {
  const iree_host_size_t count = loom_cpu_packed_dot_descriptor_count();
  ASSERT_GT(count, 0u);
  for (iree_host_size_t i = 0; i < count; ++i) {
    const loom_cpu_packed_dot_descriptor_t* lhs =
        loom_cpu_packed_dot_descriptor_at(i);
    ...
    for (iree_host_size_t j = i + 1; j < count; ++j) {
      const loom_cpu_packed_dot_descriptor_t* rhs =
          loom_cpu_packed_dot_descriptor_at(j);
      ASSERT_NE(rhs, nullptr);
      EXPECT_FALSE(iree_string_view_equal(lhs->name, rhs->name))
          << ToString(lhs->name);
    }
  }
  EXPECT_EQ(loom_cpu_packed_dot_descriptor_at(count), nullptr);
}
...
TEST(PackedDotContractTest, NamesExposeStableDisplayStrings) {
  EXPECT_EQ(ToString(loom_cpu_packed_dot_family_name(
                LOOM_CPU_PACKED_DOT_FAMILY_X86_AVX_VNNI_INT8)),
            "x86-avx-vnni-int8");
  EXPECT_EQ(ToString(loom_cpu_packed_dot_numeric_type_name(
                LOOM_CPU_PACKED_DOT_NUMERIC_U16)),
            "u16");
  EXPECT_EQ(ToString(loom_cpu_packed_dot_family_name(
                LOOM_CPU_PACKED_DOT_FAMILY_UNKNOWN)),
            "unknown");
}
...
TEST(PackedDotContractTest, Avx512VnniByteDescriptor) {
  ...
  EXPECT_EQ(descriptor->lhs_numeric_type, LOOM_CPU_PACKED_DOT_NUMERIC_U8);
  EXPECT_EQ(descriptor->rhs_numeric_type, LOOM_CPU_PACKED_DOT_NUMERIC_I8);
  EXPECT_EQ(descriptor->result_numeric_type, LOOM_CPU_PACKED_DOT_NUMERIC_I32);
}
...
TEST(PackedDotContractTest, Avx512Bf16Descriptor) {
  ...
  ASSERT_NE(descriptor, nullptr);
  EXPECT_EQ(descriptor->family, LOOM_CPU_PACKED_DOT_FAMILY_X86_AVX512_BF16);
  EXPECT_EQ(descriptor->required_feature_bits,
...
TEST(PackedDotContractTest, FeatureNamesMapToFeatureBits) {
  EXPECT_EQ(FeatureBits("x86-avx-vnni"),
  ...
  IREE_EXPECT_STATUS_IS(IREE_STATUS_INVALID_ARGUMENT,
                        loom_cpu_packed_dot_feature_bits_for_name(
...
TEST(PackedDotContractTest, SelectsAvxVnniByteDot) {
  const loom_cpu_packed_dot_match_request_t request = MatchRequest(
      LOOM_CPU_PACKED_DOT_FAMILY_UNKNOWN, 256, 32, 8, 4,
  ...
  const loom_cpu_packed_dot_descriptor_t* descriptor =
      loom_cpu_packed_dot_select(&request, &diagnostic);
  ASSERT_NE(descriptor, nullptr);
  EXPECT_EQ(ToString(descriptor->name), "x86.avx-vnni.vpdpbusd.256");
  EXPECT_EQ(diagnostic.rejection_bits, LOOM_CPU_PACKED_DOT_REJECTION_NONE);
...
TEST(PackedDotContractTest, SelectsSaturatingVariantWhenRequired) {
  const loom_cpu_packed_dot_match_request_t request = MatchRequest(
      LOOM_CPU_PACKED_DOT_FAMILY_X86_AVX_VNNI, 256, 32, 8, 4,
      LOOM_CPU_PACKED_DOT_NUMERIC_U8, LOOM_CPU_PACKED_DOT_NUMERIC_I8,
      LOOM_CPU_PACKED_DOT_NUMERIC_I32, LOOM_CPU_PACKED_DOT_NUMERIC_I32,
      FeatureBits("x86-avx-vnni"),
      LOOM_CPU_PACKED_DOT_CONTRACT_FLAG_SATURATING);

  const loom_cpu_packed_dot_descriptor_t* descriptor =
      loom_cpu_packed_dot_select(&request, nullptr);
  ASSERT_NE(descriptor, nullptr);
  EXPECT_EQ(ToString(descriptor->name), "x86.avx-vnni.vpdpbusds.256");
  EXPECT_EQ(descriptor->flags & LOOM_CPU_PACKED_DOT_CONTRACT_FLAG_SATURATING,
            LOOM_CPU_PACKED_DOT_CONTRACT_FLAG_SATURATING);
}

TEST(PackedDotContractTest, SignedSignedByteDotNeedsInt8Feature) {
  const loom_cpu_packed_dot_match_request_t avx_vnni_request = MatchRequest(
      LOOM_CPU_PACKED_DOT_FAMILY_UNKNOWN, 256, 32, 8, 4,
  ...
  loom_cpu_packed_dot_match_diagnostic_t diagnostic = {};
  EXPECT_EQ(loom_cpu_packed_dot_select(&avx_vnni_request, &diagnostic),
            nullptr);
  EXPECT_EQ(diagnostic.rejection_bits, LOOM_CPU_PACKED_DOT_REJECTION_FEATURES);
  EXPECT_GT(diagnostic.payload_candidate_count, 0u);

  const loom_cpu_packed_dot_match_request_t int8_request = MatchRequest(
      LOOM_CPU_PACKED_DOT_FAMILY_UNKNOWN, 256, 32, 8, 4,
      LOOM_CPU_PACKED_DOT_NUMERIC_I8, LOOM_CPU_PACKED_DOT_NUMERIC_I8,
      LOOM_CPU_PACKED_DOT_NUMERIC_I32, LOOM_CPU_PACKED_DOT_NUMERIC_I32,
      FeatureBits("x86-avx-vnni-int8"), 0);
  const loom_cpu_packed_dot_descriptor_t* descriptor =
      loom_cpu_packed_dot_select(&int8_request, nullptr);
  ASSERT_NE(descriptor, nullptr);
...
TEST(PackedDotContractTest, RejectsPackedI4FallbackShape) {
  const loom_cpu_packed_dot_match_request_t request = MatchRequest(
      LOOM_CPU_PACKED_DOT_FAMILY_UNKNOWN, 128, 4, 4, 8,
      LOOM_CPU_PACKED_DOT_NUMERIC_I8, LOOM_CPU_PACKED_DOT_NUMERIC_I8,
      LOOM_CPU_PACKED_DOT_NUMERIC_I32, LOOM_CPU_PACKED_DOT_NUMERIC_I32,
      FeatureBits("x86-avx-vnni-int8"), 0);
...
TEST(PackedDotContractTest, RejectsNullRequest) {
  loom_cpu_packed_dot_match_diagnostic_t diagnostic = {};
  EXPECT_EQ(loom_cpu_packed_dot_select(nullptr, &diagnostic), nullptr);
  EXPECT_EQ(diagnostic.rejection_bits,
            LOOM_CPU_PACKED_DOT_REJECTION_INVALID_REQUEST);
}
