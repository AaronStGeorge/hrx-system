...
typedef enum loom_ireevm_ref_lower_plan_e {
  LOOM_IREEVM_REF_LOWER_PLAN_RETAIN = 1,
  LOOM_IREEVM_REF_LOWER_PLAN_RELEASE = 2,
  LOOM_IREEVM_REF_LOWER_PLAN_DISCARD = 3,
} loom_ireevm_ref_lower_plan_t;
...
static iree_status_t loom_ireevm_select_op(void* user_data,
  ...
  switch (source_op->kind) {
    case LOOM_OP_IREEVM_REF_RETAIN:
      *out_plan =
          loom_low_lower_plan_make(LOOM_IREEVM_REF_LOWER_PLAN_RETAIN, NULL);
      return iree_ok_status();
    case LOOM_OP_IREEVM_REF_RELEASE:
      *out_plan =
          loom_low_lower_plan_make(LOOM_IREEVM_REF_LOWER_PLAN_RELEASE, NULL);
      return iree_ok_status();
    case LOOM_OP_IREEVM_REF_DISCARD:
      *out_plan =
          loom_low_lower_plan_make(LOOM_IREEVM_REF_LOWER_PLAN_DISCARD, NULL);
      return iree_ok_status();
    default:
...
static iree_status_t loom_ireevm_emit_op(void* user_data,
  ...
  (void)user_data;
  switch ((loom_ireevm_ref_lower_plan_t)plan.id) {
    case LOOM_IREEVM_REF_LOWER_PLAN_RETAIN:
      return loom_ireevm_emit_ref_retain(context, source_op);
    case LOOM_IREEVM_REF_LOWER_PLAN_RELEASE:
      return loom_ireevm_emit_ref_release(context, source_op);
    case LOOM_IREEVM_REF_LOWER_PLAN_DISCARD:
      return loom_ireevm_emit_ref_discard(context, source_op);
  }
  return iree_make_status(IREE_STATUS_INTERNAL,
                          "unknown IREE VM ref lowering plan");
}
