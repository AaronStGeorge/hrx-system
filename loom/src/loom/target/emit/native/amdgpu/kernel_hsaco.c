// Copyright 2026 The IREE Authors
//
// Licensed under the Apache License v2.0 with LLVM Exceptions.
// See https://llvm.org/LICENSE.txt for license information.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception

#include "loom/target/emit/native/amdgpu/kernel_hsaco.h"

#include <inttypes.h>
#include <string.h>

#include "loom/codegen/low/packet.h"
#include "loom/ops/low/ops.h"
#include "loom/target/arch/amdgpu/hal_kernel_abi.h"
#include "loom/target/arch/amdgpu/target_info.h"
#include "loom/target/emit/native/amdgpu/encoding.h"
#include "loom/target/emit/native/amdgpu/hsaco.h"

#define LOOM_AMDGPU_KERNEL_HSACO_KERNARG_USER_SGPR_COUNT 2u

typedef struct loom_amdgpu_kernel_hsaco_register_usage_t {
  // Highest SGPR index used by the function body plus one.
  uint32_t next_free_sgpr;
  // Highest VGPR index used by the function body plus one.
  uint32_t next_free_vgpr;
} loom_amdgpu_kernel_hsaco_register_usage_t;

static iree_status_t loom_amdgpu_kernel_hsaco_concat3(
    iree_string_view_t a, iree_string_view_t b, iree_string_view_t c,
    iree_string_view_t* out_value, iree_arena_allocator_t* arena) {
  IREE_ASSERT_ARGUMENT(out_value);
  *out_value = iree_string_view_empty();
  iree_host_size_t prefix_length = 0;
  iree_host_size_t length = 0;
  if (!iree_host_size_checked_add(a.size, b.size, &prefix_length) ||
      !iree_host_size_checked_add(prefix_length, c.size, &length)) {
    return iree_make_status(IREE_STATUS_OUT_OF_RANGE,
                            "AMDGPU kernel HSACO string length overflows");
  }
  char* data = NULL;
  IREE_RETURN_IF_ERROR(iree_arena_allocate(arena, length, (void**)&data));
  char* cursor = data;
  memcpy(cursor, a.data, a.size);
  cursor += a.size;
  memcpy(cursor, b.data, b.size);
  cursor += b.size;
  memcpy(cursor, c.data, c.size);
  *out_value = iree_make_string_view(data, length);
  return iree_ok_status();
}

static bool loom_amdgpu_kernel_hsaco_symbol_start_char(char c) {
  return (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '_' ||
         c == '$';
}

static bool loom_amdgpu_kernel_hsaco_symbol_continue_char(char c) {
  return loom_amdgpu_kernel_hsaco_symbol_start_char(c) ||
         (c >= '0' && c <= '9') || c == '.';
}

static iree_status_t loom_amdgpu_kernel_hsaco_validate_symbol(
    iree_string_view_t symbol) {
  if (iree_string_view_is_empty(symbol)) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "AMDGPU kernel HSACO symbol is required");
  }
  if (!loom_amdgpu_kernel_hsaco_symbol_start_char(symbol.data[0])) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "AMDGPU kernel HSACO symbol '%.*s' has an invalid first character",
        (int)symbol.size, symbol.data);
  }
  for (iree_host_size_t i = 1; i < symbol.size; ++i) {
    if (!loom_amdgpu_kernel_hsaco_symbol_continue_char(symbol.data[i])) {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "AMDGPU kernel HSACO symbol '%.*s' contains an invalid character",
          (int)symbol.size, symbol.data);
    }
  }
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_kernel_hsaco_validate_target_id_component(
    iree_string_view_t value, iree_string_view_t field_name) {
  if (iree_string_view_is_empty(value)) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "AMDGPU kernel HSACO target-id field '%.*s' is required",
        (int)field_name.size, field_name.data);
  }
  for (iree_host_size_t i = 0; i < value.size; ++i) {
    const unsigned char c = (unsigned char)value.data[i];
    if (c <= ' ' || c == '"' || c == '\\') {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "AMDGPU kernel HSACO target-id field '%.*s' contains an unsupported "
          "character",
          (int)field_name.size, field_name.data);
    }
  }
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_kernel_hsaco_symbol_name(
    const loom_module_t* module, const loom_op_t* function_op,
    iree_string_view_t* out_symbol) {
  IREE_ASSERT_ARGUMENT(out_symbol);
  *out_symbol = iree_string_view_empty();
  loom_symbol_ref_t symbol_ref = loom_low_func_def_callee(function_op);
  if (!loom_symbol_ref_is_valid(symbol_ref) || symbol_ref.module_id != 0 ||
      symbol_ref.symbol_id >= module->symbols.count) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "AMDGPU kernel HSACO function symbol is invalid");
  }
  const loom_symbol_t* symbol = &module->symbols.entries[symbol_ref.symbol_id];
  if (symbol->name_id == LOOM_STRING_ID_INVALID ||
      symbol->name_id >= module->strings.count) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "AMDGPU kernel HSACO function symbol has no "
                            "module string");
  }
  *out_symbol = module->strings.entries[symbol->name_id];
  return loom_amdgpu_kernel_hsaco_validate_symbol(*out_symbol);
}

static iree_status_t loom_amdgpu_kernel_hsaco_export_symbol(
    const loom_low_resolved_target_t* target, const loom_module_t* module,
    const loom_op_t* function_op, iree_string_view_t* out_symbol) {
  IREE_ASSERT_ARGUMENT(out_symbol);
  if (!iree_string_view_is_empty(
          target->bundle_storage.export_plan.export_symbol)) {
    *out_symbol = target->bundle_storage.export_plan.export_symbol;
    return loom_amdgpu_kernel_hsaco_validate_symbol(*out_symbol);
  }
  return loom_amdgpu_kernel_hsaco_symbol_name(module, function_op, out_symbol);
}

static iree_status_t loom_amdgpu_kernel_hsaco_validate_target(
    const loom_low_resolved_target_t* target) {
  const loom_target_snapshot_t* snapshot = &target->bundle_storage.snapshot;
  const loom_target_export_plan_t* export_plan =
      &target->bundle_storage.export_plan;
  if (target->descriptor_set == NULL) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "AMDGPU kernel HSACO target bundle is required");
  }
  if (snapshot->codegen_format != LOOM_TARGET_CODEGEN_FORMAT_LOW_NATIVE) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "AMDGPU kernel HSACO requires low_native codegen snapshots");
  }
  if (!iree_string_view_equal(snapshot->target_triple,
                              IREE_SV("amdgcn-amd-amdhsa"))) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "AMDGPU kernel HSACO requires amdgcn-amd-amdhsa, got '%.*s'",
        (int)snapshot->target_triple.size, snapshot->target_triple.data);
  }
  if (!iree_string_view_is_empty(snapshot->target_features)) {
    return iree_make_status(
        IREE_STATUS_UNIMPLEMENTED,
        "AMDGPU kernel HSACO target-id feature strings are not encoded yet");
  }
  if (snapshot->artifact_format != LOOM_TARGET_ARTIFACT_FORMAT_ELF) {
    return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
                            "AMDGPU kernel HSACO requires ELF artifacts");
  }
  if (export_plan->abi_kind != LOOM_TARGET_ABI_HAL_KERNEL) {
    return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                            "AMDGPU kernel HSACO requires a HAL kernel ABI");
  }
  if (export_plan->linkage != LOOM_TARGET_LINKAGE_DEFAULT) {
    return iree_make_status(
        IREE_STATUS_UNIMPLEMENTED,
        "AMDGPU kernel HSACO currently requires default linkage");
  }
  IREE_RETURN_IF_ERROR(loom_amdgpu_kernel_hsaco_validate_target_id_component(
      snapshot->target_triple, IREE_SV("target_triple")));
  return loom_amdgpu_kernel_hsaco_validate_target_id_component(
      snapshot->target_cpu, IREE_SV("target_cpu"));
}

static iree_status_t loom_amdgpu_kernel_hsaco_validate_export_source(
    const loom_low_resolved_target_t* target, const loom_module_t* module,
    const loom_op_t* function_op) {
  iree_string_view_t source_symbol =
      target->bundle_storage.export_plan.source_symbol;
  if (iree_string_view_is_empty(source_symbol)) {
    return iree_ok_status();
  }
  iree_string_view_t function_symbol = iree_string_view_empty();
  IREE_RETURN_IF_ERROR(loom_amdgpu_kernel_hsaco_symbol_name(module, function_op,
                                                            &function_symbol));
  if (!iree_string_view_equal(source_symbol, function_symbol)) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "AMDGPU kernel HSACO export plan source '%.*s' does not match low "
        "function '%.*s'",
        (int)source_symbol.size, source_symbol.data, (int)function_symbol.size,
        function_symbol.data);
  }
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_kernel_hsaco_validate_function_shape(
    const loom_op_t* function_op) {
  if (!loom_low_func_def_isa(function_op)) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "AMDGPU kernel HSACO requires low.func.def");
  }
  loom_region_t* body = loom_low_func_def_body(function_op);
  if (body == NULL || body->block_count == 0) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "AMDGPU kernel HSACO function body is required");
  }
  const loom_block_t* entry_block = loom_region_const_entry_block(body);
  if (entry_block->arg_count != 0 || function_op->result_count != 0) {
    return iree_make_status(
        IREE_STATUS_UNIMPLEMENTED,
        "AMDGPU kernel HSACO currently requires ABI-lowered kernels with no "
        "low function arguments or results");
  }
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_kernel_hsaco_update_high_water(
    uint32_t location_base, uint32_t location_count, uint32_t* inout_value) {
  IREE_ASSERT_ARGUMENT(inout_value);
  uint64_t next_free = (uint64_t)location_base + location_count;
  if (next_free > UINT32_MAX) {
    return iree_make_status(
        IREE_STATUS_OUT_OF_RANGE,
        "AMDGPU kernel HSACO register high-water mark overflows");
  }
  if ((uint32_t)next_free > *inout_value) {
    *inout_value = (uint32_t)next_free;
  }
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_kernel_hsaco_collect_register_usage(
    const loom_low_allocation_sidecar_t* allocation,
    loom_amdgpu_kernel_hsaco_register_usage_t* out_usage) {
  IREE_ASSERT_ARGUMENT(out_usage);
  *out_usage = (loom_amdgpu_kernel_hsaco_register_usage_t){0};
  if (allocation->spill_plan_count != 0 || allocation->spill_count != 0) {
    return iree_make_status(
        IREE_STATUS_UNIMPLEMENTED,
        "AMDGPU kernel HSACO requires materialized spill lowering before "
        "spilled allocations can be emitted");
  }
  for (iree_host_size_t i = 0; i < allocation->assignment_count; ++i) {
    const loom_low_allocation_assignment_t* assignment =
        &allocation->assignments[i];
    if (assignment->value_class.type_kind != LOOM_TYPE_REGISTER) {
      continue;
    }
    if (assignment->location_kind !=
        LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER) {
      return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
                              "AMDGPU kernel HSACO value %" PRIu32
                              " is not physically allocated",
                              assignment->value_id);
    }
    if (assignment->location_count == 0) {
      return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
                              "AMDGPU kernel HSACO value %" PRIu32
                              " has an empty physical register range",
                              assignment->value_id);
    }
    if (assignment->value_class.register_class_id == LOOM_STRING_ID_INVALID ||
        assignment->value_class.register_class_id >=
            allocation->module->strings.count) {
      return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
                              "AMDGPU kernel HSACO value %" PRIu32
                              " has no register-class string",
                              assignment->value_id);
    }
    iree_string_view_t register_class =
        allocation->module->strings
            .entries[assignment->value_class.register_class_id];
    if (iree_string_view_equal(register_class, IREE_SV("amdgpu.sgpr"))) {
      IREE_RETURN_IF_ERROR(loom_amdgpu_kernel_hsaco_update_high_water(
          assignment->location_base, assignment->location_count,
          &out_usage->next_free_sgpr));
      continue;
    }
    if (iree_string_view_equal(register_class, IREE_SV("amdgpu.vgpr"))) {
      IREE_RETURN_IF_ERROR(loom_amdgpu_kernel_hsaco_update_high_water(
          assignment->location_base, assignment->location_count,
          &out_usage->next_free_vgpr));
      continue;
    }
    if (iree_string_view_starts_with(register_class, IREE_SV("amdgpu."))) {
      return iree_make_status(
          IREE_STATUS_UNIMPLEMENTED,
          "AMDGPU kernel HSACO register class '%.*s' requires additional "
          "kernel descriptor metadata",
          (int)register_class.size, register_class.data);
    }
  }
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_kernel_hsaco_validate_kernarg_live_in(
    const loom_low_allocation_sidecar_t* allocation,
    const loom_amdgpu_hal_kernel_abi_layout_t* abi_layout) {
  if (!abi_layout->uses_kernarg_segment_ptr) {
    return iree_ok_status();
  }
  bool found = false;
  for (iree_host_size_t i = 0; i < allocation->assignment_count; ++i) {
    const loom_low_allocation_assignment_t* assignment =
        &allocation->assignments[i];
    if (assignment->value_class.type_kind != LOOM_TYPE_REGISTER ||
        assignment->location_kind !=
            LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER) {
      continue;
    }
    if (!loom_amdgpu_hal_kernel_abi_is_kernarg_segment_ptr_live_in(
            allocation->module, assignment->value_id)) {
      continue;
    }
    found = true;
    if (assignment->location_base != 0 ||
        assignment->location_count !=
            LOOM_AMDGPU_KERNEL_HSACO_KERNARG_USER_SGPR_COUNT) {
      return iree_make_status(
          IREE_STATUS_FAILED_PRECONDITION,
          "AMDGPU kernel HSACO requires the kernarg segment pointer live-in "
          "to be fixed to s[0:%u]",
          LOOM_AMDGPU_KERNEL_HSACO_KERNARG_USER_SGPR_COUNT - 1);
    }
  }
  if (!found) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION,
        "AMDGPU kernel HSACO requires a kernarg segment pointer live-in for "
        "kernarg-using kernels");
  }
  return iree_ok_status();
}

static iree_status_t
loom_amdgpu_kernel_hsaco_collect_descriptor_options_from_live_ins(
    const loom_low_allocation_sidecar_t* allocation,
    loom_amdgpu_hsaco_kernel_descriptor_options_t* out_options) {
  IREE_ASSERT_ARGUMENT(out_options);
  *out_options = (loom_amdgpu_hsaco_kernel_descriptor_options_t){0};
  for (iree_host_size_t i = 0; i < allocation->assignment_count; ++i) {
    const loom_low_allocation_assignment_t* assignment =
        &allocation->assignments[i];
    if (assignment->value_class.type_kind != LOOM_TYPE_REGISTER ||
        assignment->location_kind !=
            LOOM_LOW_ALLOCATION_LOCATION_PHYSICAL_REGISTER) {
      continue;
    }
    if (!loom_amdgpu_hal_kernel_abi_is_workitem_id_x_live_in(
            allocation->module, assignment->value_id)) {
      continue;
    }
    if (assignment->location_base != 0 || assignment->location_count != 1) {
      return iree_make_status(
          IREE_STATUS_FAILED_PRECONDITION,
          "AMDGPU kernel HSACO requires the workitem_id.x live-in to be fixed "
          "to v0");
    }
    out_options->flags |=
        LOOM_AMDGPU_KERNEL_DESCRIPTOR_SYSTEM_VGPR_WORKITEM_ID_X;
  }
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_kernel_hsaco_build_metadata_arguments(
    const loom_amdgpu_hal_kernel_abi_layout_t* abi_layout,
    const loom_amdgpu_metadata_argument_t** out_arguments,
    iree_arena_allocator_t* arena) {
  IREE_ASSERT_ARGUMENT(out_arguments);
  *out_arguments = NULL;
  if (abi_layout->resource_count == 0) {
    return iree_ok_status();
  }
  loom_amdgpu_metadata_argument_t* arguments = NULL;
  IREE_RETURN_IF_ERROR(
      iree_arena_allocate_array(arena, abi_layout->resource_count,
                                sizeof(*arguments), (void**)&arguments));
  for (iree_host_size_t i = 0; i < abi_layout->resource_count; ++i) {
    const loom_amdgpu_hal_kernarg_resource_t* resource =
        &abi_layout->resources[i];
    arguments[i] = (loom_amdgpu_metadata_argument_t){
        .name = resource->name,
        .offset = resource->kernarg_offset,
        .size = resource->kernarg_size,
        .alignment = resource->kernarg_alignment,
        .kind = LOOM_AMDGPU_METADATA_ARGUMENT_GLOBAL_BUFFER,
        .address_space = IREE_SV("global"),
    };
  }
  *out_arguments = arguments;
  return iree_ok_status();
}

static iree_status_t loom_amdgpu_emit_kernel_hsaco_internal(
    const loom_low_schedule_sidecar_t* schedule,
    const loom_low_allocation_sidecar_t* allocation,
    const loom_amdgpu_wait_packet_plan_t* wait_packets,
    iree_io_stream_t* stream, iree_arena_allocator_t* scratch_arena) {
  if (stream == NULL || scratch_arena == NULL) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "AMDGPU kernel HSACO output stream and scratch arena are required");
  }
  IREE_RETURN_IF_ERROR(loom_low_packet_validate_sidecars(schedule, allocation));
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_kernel_hsaco_validate_target(&schedule->target));
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_kernel_hsaco_validate_function_shape(schedule->function_op));
  IREE_RETURN_IF_ERROR(loom_amdgpu_kernel_hsaco_validate_export_source(
      &schedule->target, schedule->module, schedule->function_op));

  iree_string_view_t symbol = iree_string_view_empty();
  IREE_RETURN_IF_ERROR(loom_amdgpu_kernel_hsaco_export_symbol(
      &schedule->target, schedule->module, schedule->function_op, &symbol));

  loom_amdgpu_hal_kernel_abi_layout_t abi_layout = {0};
  IREE_RETURN_IF_ERROR(loom_amdgpu_hal_kernel_abi_layout_from_low(
      schedule->module, schedule->function_op,
      &schedule->target.bundle_storage.bundle, &abi_layout, scratch_arena));
  loom_amdgpu_kernel_hsaco_register_usage_t register_usage = {0};
  IREE_RETURN_IF_ERROR(loom_amdgpu_kernel_hsaco_collect_register_usage(
      allocation, &register_usage));
  IREE_RETURN_IF_ERROR(loom_amdgpu_kernel_hsaco_validate_kernarg_live_in(
      allocation, &abi_layout));
  loom_amdgpu_hsaco_kernel_descriptor_options_t descriptor_options = {0};
  IREE_RETURN_IF_ERROR(
      loom_amdgpu_kernel_hsaco_collect_descriptor_options_from_live_ins(
          allocation, &descriptor_options));

  const uint32_t user_sgpr_count =
      abi_layout.uses_kernarg_segment_ptr
          ? LOOM_AMDGPU_KERNEL_HSACO_KERNARG_USER_SGPR_COUNT
          : 0;
  const uint32_t next_free_sgpr =
      register_usage.next_free_sgpr > user_sgpr_count
          ? register_usage.next_free_sgpr
          : user_sgpr_count;

  const loom_target_snapshot_t* snapshot =
      &schedule->target.bundle_storage.snapshot;
  const loom_target_hal_kernel_abi_t* hal_kernel =
      &schedule->target.bundle_storage.export_plan.hal_kernel;
  const loom_amdgpu_processor_info_t* processor = NULL;
  IREE_RETURN_IF_ERROR(loom_amdgpu_target_info_lookup_processor(
      snapshot->target_cpu, &processor));

  iree_const_byte_span_t text = iree_const_byte_span_empty();
  if (wait_packets != NULL) {
    IREE_RETURN_IF_ERROR(
        loom_amdgpu_encode_instruction_stream_with_wait_packets(
            schedule, allocation, wait_packets, &text, scratch_arena));
  } else {
    IREE_RETURN_IF_ERROR(loom_amdgpu_encode_instruction_stream(
        schedule, allocation, &text, scratch_arena));
  }

  const loom_amdgpu_metadata_argument_t* arguments = NULL;
  iree_string_view_t target_id = iree_string_view_empty();
  iree_status_t status = loom_amdgpu_kernel_hsaco_concat3(
      snapshot->target_triple, IREE_SV("--"), snapshot->target_cpu, &target_id,
      scratch_arena);
  iree_string_view_t descriptor_symbol = iree_string_view_empty();
  if (iree_status_is_ok(status)) {
    status = loom_amdgpu_kernel_hsaco_concat3(
        symbol, IREE_SV(".kd"), iree_string_view_empty(), &descriptor_symbol,
        scratch_arena);
  }
  if (iree_status_is_ok(status)) {
    status = loom_amdgpu_kernel_hsaco_build_metadata_arguments(
        &abi_layout, &arguments, scratch_arena);
  }
  if (iree_status_is_ok(status)) {
    loom_amdgpu_metadata_kernel_t metadata = {
        .name = symbol,
        .descriptor_symbol = descriptor_symbol,
        .kernarg_segment_size = abi_layout.kernarg_segment_size,
        .kernarg_segment_alignment = abi_layout.kernarg_segment_alignment,
        .wavefront_size = processor->default_wavefront_size,
        .group_segment_fixed_size = 0,
        .private_segment_fixed_size = 0,
        .sgpr_count = next_free_sgpr,
        .vgpr_count = register_usage.next_free_vgpr,
        .max_flat_workgroup_size = hal_kernel->flat_workgroup_size_max,
        .required_workgroup_size = hal_kernel->required_workgroup_size,
        .has_required_workgroup_size = true,
        .arguments = arguments,
        .argument_count = abi_layout.resource_count,
    };
    const loom_amdgpu_hsaco_kernel_t kernel = {
        .metadata = metadata,
        .descriptor_options = descriptor_options,
        .text = text,
    };
    const loom_amdgpu_hsaco_file_t file = {
        .target = target_id,
        .target_cpu = snapshot->target_cpu,
        .kernels = &kernel,
        .kernel_count = 1,
    };
    status = loom_amdgpu_hsaco_write_file(&file, stream, scratch_arena);
  }
  return status;
}

iree_status_t loom_amdgpu_emit_kernel_hsaco(
    const loom_low_schedule_sidecar_t* schedule,
    const loom_low_allocation_sidecar_t* allocation, iree_io_stream_t* stream,
    iree_arena_allocator_t* scratch_arena) {
  return loom_amdgpu_emit_kernel_hsaco_internal(schedule, allocation, NULL,
                                                stream, scratch_arena);
}

iree_status_t loom_amdgpu_emit_kernel_hsaco_with_wait_packets(
    const loom_low_schedule_sidecar_t* schedule,
    const loom_low_allocation_sidecar_t* allocation,
    const loom_amdgpu_wait_packet_plan_t* wait_packets,
    iree_io_stream_t* stream, iree_arena_allocator_t* scratch_arena) {
  return loom_amdgpu_emit_kernel_hsaco_internal(
      schedule, allocation, wait_packets, stream, scratch_arena);
}
