...
#define LOOM_AMDGPU_SOPP_S_ENDPGM_OPCODE UINT16_C(0x30)
...
static iree_status_t loom_amdgpu_encode_return_packet(
  ...
  return loom_amdgpu_encode_sopp_word(state, LOOM_AMDGPU_SOPP_S_ENDPGM_OPCODE,
                                      0);
}
...
static iree_status_t loom_amdgpu_verify_encoding_target(
    const loom_low_schedule_sidecar_t* schedule) {
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_descriptor_string(
      schedule->target.descriptor_set,
      schedule->target.descriptor_set->key_string_offset, &set_key));
  if (!iree_string_view_equal(set_key, IREE_SV("amdgpu.gfx11.core"))) {
    return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                            "AMDGPU native encoding requires descriptor set "
                            "amdgpu.gfx11.core but received '%.*s'",
...
static iree_status_t loom_amdgpu_encode_instruction_stream_internal(
  ...
  *out_text = iree_const_byte_span_empty();
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_verify_encoding_target(schedule));
