  ...
  *out_value = LOOM_VALUE_ID_INVALID;
  ...
  if (loom_amdgpu_u32_is_power_of_two(scale)) {
    ...
    IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
        context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_S_MOV_B32, shift,
        lane_type, &low_shift));
    return loom_amdgpu_emit_sgpr_binary(context, source_op,
                                        LOOM_AMDGPU_DESCRIPTOR_REF_S_LSHL_B32,
                                        value, low_shift, lane_type, out_value);
  }

  loom_value_id_t low_scale = LOOM_VALUE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_amdgpu_emit_const_u32(
      context, source_op, LOOM_AMDGPU_DESCRIPTOR_REF_S_MOV_B32, scale,
      lane_type, &low_scale));
  return loom_amdgpu_emit_sgpr_mul_u32(context, source_op, value, low_scale,
                                       lane_type, out_value);
}
...
iree_status_t loom_amdgpu_materialize_low_vgpr_b32_registers(
  ...
  *out_low_value = low_value;
  const loom_module_t* module = loom_low_lower_context_module(context);
  const loom_type_t low_type = loom_module_value_type(module, low_value);
  const uint32_t unit_count = loom_type_register_unit_count(low_type);
  bool is_vgpr = false;
  IREE_RETURN_IF_ERROR(loom_amdgpu_low_type_register_class_is(
      context, low_type, LOOM_AMDGPU_REG_CLASS_ID_VGPR, &is_vgpr));
...
iree_status_t loom_amdgpu_lookup_or_materialize_native_i1_mask(
  ...
  *out_low_value = LOOM_VALUE_ID_INVALID;
  ...
  IREE_RETURN_IF_ERROR(loom_amdgpu_low_type_register_class_is(
      context, low_type, LOOM_AMDGPU_REG_CLASS_ID_SGPR, &is_sgpr));
  if (is_sgpr && loom_type_register_unit_count(low_type) == 2) {
    *out_low_value = low_value;
    return iree_ok_status();
  }
