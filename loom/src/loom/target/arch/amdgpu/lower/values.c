...
static iree_status_t loom_amdgpu_select_vector_constant_plan(
  ...
  *out_selected = false;
  ...
  if (i32_register_count != 0) {
    ...
    out_plan->register_count = i32_register_count;
    return iree_ok_status();
  }
...
static bool loom_amdgpu_select_vector_from_elements_plan(
  ...
  *out_plan = (loom_amdgpu_vector_from_elements_plan_t){0};
  ...
  if (elements.count == 0 ||
      elements.count > LOOM_AMDGPU_MAX_PACKED_16BIT_FLOAT_LANES) {
    return false;
  }
  ...
  uint32_t register_count =
      loom_amdgpu_vector_32bit_register_count(result_type);
  if (register_count != 0) {
    if (elements.count > LOOM_AMDGPU_MAX_SCALARIZED_32BIT_LANES ||
        register_count != elements.count ||
    ...
    uint32_t payload_bit_count = 0;
    if (!loom_amdgpu_type_packed_16bit_float_storage(
            result_type, &payload_bit_count, &register_count) ||
        payload_bit_count != elements.count * 16u) {
      return false;
    }
  }
...
static bool loom_amdgpu_select_vector_splat_plan(
  ...
  *out_plan = (loom_amdgpu_vector_from_elements_plan_t){0};
  ...
  const uint32_t element_count =
      loom_amdgpu_vector_32bit_register_count(result_type);
  if (element_count == 0 ||
      element_count > LOOM_AMDGPU_MAX_SCALARIZED_32BIT_LANES) {
    return false;
  }
  const loom_scalar_type_t element_type = loom_type_element_type(result_type);
  if (element_type != LOOM_SCALAR_TYPE_I32 &&
  ...
  out_plan->result = result;
  out_plan->element_count = element_count;
  out_plan->element_type = element_type;
  return true;
}
  ...
  for (uint32_t register_index = 0;
    ...
    if (lane_base + 1u < plan->lane_count) {
      IREE_RETURN_IF_ERROR(loom_amdgpu_extract_register_unit(
          ...
          lane_base + 1u, source_lane_type, &source_lane));
      loom_value_id_t high_lane = LOOM_VALUE_ID_INVALID;
      IREE_RETURN_IF_ERROR(loom_amdgpu_emit_f32_to_bf16_lane(
          context, source_op, source_lane, lane_type, &high_lane));
