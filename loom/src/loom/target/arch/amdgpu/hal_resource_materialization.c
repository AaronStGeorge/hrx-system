...
iree_status_t loom_amdgpu_hal_resource_fixed_values_from_low(
  ...
  *out_fixed_value_count = 0;
  if (module == NULL || function_op == NULL || arena == NULL) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "AMDGPU HAL resource fixed-value collection requires a module and "
        "function, and arena");
  }
  if (!loom_low_func_def_isa(function_op)) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "AMDGPU HAL resource fixed-value collection requires low.func.def");
  }
  ...
  if (body == NULL || body->block_count == 0) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "AMDGPU HAL resource fixed-value collection requires a function body");
  }
  ...
  loom_block_for_each_op(entry_block, op) {
    ...
    if (kernarg_ptr != LOOM_VALUE_ID_INVALID) {
      return iree_make_status(
          IREE_STATUS_ALREADY_EXISTS,
          "AMDGPU HAL resource fixed-value collection found duplicate kernarg "
          "segment pointer live-ins");
    }
    kernarg_ptr = loom_low_live_in_result(op);
    IREE_RETURN_IF_ERROR(loom_amdgpu_hal_resource_verify_sgpr_range_value(
        module, kernarg_ptr, 2, IREE_SV("kernarg live-in")));
  }
  ...
  IREE_RETURN_IF_ERROR(iree_arena_allocate_array(
      arena, 1, sizeof(*fixed_values), (void**)&fixed_values));
  fixed_values[0] = (loom_low_allocation_fixed_value_t){
