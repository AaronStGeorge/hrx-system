...
struct ViewPayload {
  ...
  // Target-native payload shape used for contract matching.
  loom_amdgpu_matrix_payload_shape_t payload = {};
  // Scale operand shape required by encoded matrix storage, if any.
  loom_amdgpu_matrix_scale_kind_t scale_kind = LOOM_AMDGPU_MATRIX_SCALE_NONE;
  // Descriptor flag classes proven available from the view type/facts.
  loom_amdgpu_matrix_contract_flags_t available_flags = 0;
  ...
  *out_payload = {};
  switch (element_type) {
    case LOOM_SCALAR_TYPE_I8:
      *out_payload = PayloadShape(LOOM_AMDGPU_MATRIX_NUMERIC_I8);
      return true;
    case LOOM_SCALAR_TYPE_I32:
      *out_payload = PayloadShape(LOOM_AMDGPU_MATRIX_NUMERIC_I32);
      return true;
    case LOOM_SCALAR_TYPE_F16:
      *out_payload = PayloadShape(LOOM_AMDGPU_MATRIX_NUMERIC_F16);
      return true;
    case LOOM_SCALAR_TYPE_BF16:
      *out_payload = PayloadShape(LOOM_AMDGPU_MATRIX_NUMERIC_BF16);
      return true;
    case LOOM_SCALAR_TYPE_F32:
      *out_payload = PayloadShape(LOOM_AMDGPU_MATRIX_NUMERIC_F32);
      return true;
    default:
      return false;
  }
...
bool QueryViewPayload(const loom_value_fact_table_t* fact_table,
  ...
  *out_payload = {};
  if (!loom_type_is_view(view_type)) return false;
  ...
  if (loom_encoding_query_type_storage_schema(&fact_table->context, module,
    ...
    if (!loom_amdgpu_matrix_payload_from_storage_schema(
            storage_schema, &out_payload->payload)) {
      return true;
    }
    if (!loom_amdgpu_matrix_scale_kind_from_storage_schema(
            storage_schema, &out_payload->scale_kind)) {
      return false;
    }
    out_payload->kind = ViewPayloadKind::kMatrixStorageSchema;
    out_payload->available_flags =
        loom_amdgpu_matrix_available_flags_from_storage_schema(storage_schema);
    return true;
  }
 ...
 protected:
  ...
  loom_func_like_t FirstFunctionWithBody(const loom_module_t* module) {
    ...
    loom_module_for_each_symbol(module, symbol) {
      ...
      if (loom_func_like_body(function)) return function;
    }
  ...
  std::vector<loom_type_t> CollectBufferViewTypes(const loom_module_t* module,
    ...
    if (!body) return view_types;

    loom_block_t* block = nullptr;
    loom_region_for_each_block(body, block) {
      loom_op_t* op = nullptr;
      loom_block_for_each_op(block, op) {
        if (!loom_buffer_view_isa(op)) continue;
...
func.def @i8_mfma(%buffer: buffer, %base: offset) {
  ...
  EXPECT_EQ(lhs.payload.numeric_type, LOOM_AMDGPU_MATRIX_NUMERIC_I8);
  EXPECT_EQ(rhs.payload.numeric_type, LOOM_AMDGPU_MATRIX_NUMERIC_I8);
  ...
  request.tile_shape.reduction_count = 16;
  request.lhs_payload = lhs.payload;
  request.rhs_payload = rhs.payload;
  request.accumulator_payload = PayloadShape(LOOM_AMDGPU_MATRIX_NUMERIC_I32);
  request.result_payload = PayloadShape(LOOM_AMDGPU_MATRIX_NUMERIC_I32);
  request.scale_kind = LOOM_AMDGPU_MATRIX_SCALE_NONE;
...
func.def @scaled_fp6_mfma(%buffer: buffer, %base: offset) {
  ...
  EXPECT_EQ(lhs.payload.numeric_type, LOOM_AMDGPU_MATRIX_NUMERIC_FP6);
  EXPECT_EQ(rhs.payload.numeric_type, LOOM_AMDGPU_MATRIX_NUMERIC_BF6);
  EXPECT_EQ(lhs.scale_kind, LOOM_AMDGPU_MATRIX_SCALE_32);
  EXPECT_EQ(rhs.scale_kind, LOOM_AMDGPU_MATRIX_SCALE_32);
  ...
  request.scale_kind = lhs.scale_kind;
  request.feature_bits = feature_bits;
  request.wave_size = 64;
  request.available_flags = lhs.available_flags | rhs.available_flags;
...
func.def @ggml_q4_reference(%buffer: buffer, %base: offset) {
  ...
  EXPECT_EQ(weights.payload.numeric_type, LOOM_AMDGPU_MATRIX_NUMERIC_UNKNOWN);
}
