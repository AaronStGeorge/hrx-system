...
func.def @f16_dot2(%lhs: vector<32xf16>, %rhs: vector<32xf16>, %acc: vector<16xf32>) -> (vector<16xf32>) {
  ...
  ASSERT_TRUE(loom_x86_packed_dot_match_request_from_vector_op(module_ptr.get(),
                                                               op, &request));
...
func.def @s8s8_dot4_wide(%lhs: vector<64xi8>, %rhs: vector<64xi8>, %acc: vector<16xi32>) -> (vector<16xi32>) {
  ...
  ASSERT_TRUE(loom_x86_packed_dot_match_request_from_vector_op(module_ptr.get(),
                                                               op, &request));
  request.family = LOOM_X86_PACKED_DOT_FAMILY_AVX10_2;
  IREE_ASSERT_OK(loom_x86_packed_dot_feature_bits_for_name(
      IREE_SV("x86-avx10.2"), &request.feature_bits));
...
func.def @packed_i4_dot(%lhs: vector<4xi32>, %rhs: vector<4xi32>, %acc: vector<4xi32>) -> (vector<4xi32>) {
  ...
  ASSERT_NE(op, nullptr);
  loom_x86_packed_dot_match_request_t request = {};
  EXPECT_FALSE(loom_x86_packed_dot_match_request_from_vector_op(
