...
// Lowering for CPU-native Loom vector contracts into structured LLVMIR ops.

#include "loom/ops/vector/ops.h"
#include "loom/target/cpu/packed_dot_vector.h"
#include "loom/target/llvmir/lower_internal.h"

static iree_status_t loom_llvmir_lowering_cpu_packed_dot_scalar_type(
  ...
  switch (numeric_type) {
    ...
    default:
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "unknown CPU packed-dot numeric type");
  }
}

static iree_status_t loom_llvmir_lowering_cpu_packed_dot_vector_type(
    loom_llvmir_lowering_state_t* state,
    loom_cpu_packed_dot_numeric_type_t numeric_type, uint16_t lane_count,
    loom_llvmir_type_id_t* out_type) {
  loom_llvmir_type_id_t element_type = LOOM_LLVMIR_TYPE_ID_INVALID;
  IREE_RETURN_IF_ERROR(loom_llvmir_lowering_cpu_packed_dot_scalar_type(
...
static iree_status_t loom_llvmir_lowering_declare_cpu_packed_dot_intrinsic(
  ...
  for (iree_host_size_t i = 0;
       i < state->cpu_packed_dot_intrinsic_function_count; ++i) {
    if (state->cpu_packed_dot_intrinsic_keys[i] == descriptor) {
      *out_function = state->cpu_packed_dot_intrinsic_functions[i];
      return iree_ok_status();
    }
  }
  if (state->cpu_packed_dot_intrinsic_function_count >=
      IREE_ARRAYSIZE(state->cpu_packed_dot_intrinsic_functions)) {
    return iree_make_status(
        IREE_STATUS_RESOURCE_EXHAUSTED,
        "LLVM CPU packed-dot intrinsic cache capacity exceeded");
  }
  ...
  iree_host_size_t cache_ordinal =
      state->cpu_packed_dot_intrinsic_function_count++;
  state->cpu_packed_dot_intrinsic_keys[cache_ordinal] = descriptor;
  state->cpu_packed_dot_intrinsic_functions[cache_ordinal] = function;
  *out_function = function;
  return iree_ok_status();
}

static const char* loom_llvmir_lowering_cpu_packed_dot_rejection_detail(
    loom_cpu_packed_dot_rejection_bits_t rejection_bits) {
  if (iree_any_bit_set(rejection_bits,
                       LOOM_CPU_PACKED_DOT_REJECTION_FEATURES)) {
    return "target profile does not enable a matching CPU packed-dot feature";
  }
  if (iree_any_bit_set(rejection_bits, LOOM_CPU_PACKED_DOT_REJECTION_PAYLOAD)) {
    return "no CPU packed-dot descriptor matches the vector dot payload types";
  }
  if (iree_any_bit_set(rejection_bits, LOOM_CPU_PACKED_DOT_REJECTION_SHAPE)) {
    return "no CPU packed-dot descriptor matches the vector dot shape";
  }
  return "no CPU packed-dot descriptor matches this vector dot op";
}
...
iree_status_t loom_llvmir_lowering_lower_vector_cpu_packed_dot(
  ...
  if (!loom_cpu_packed_dot_match_request_from_vector_op(state->source_module,
                                                        op, &request)) {
    return loom_llvmir_lowering_unsupported_op(
        state, op, "vector dot op is not a CPU packed-dot register contract");
  }
  request.feature_bits = (loom_cpu_packed_dot_feature_bits_t)
                             state->target_profile->cpu_packed_dot_feature_bits;

  loom_cpu_packed_dot_match_diagnostic_t diagnostic = {0};
  const loom_cpu_packed_dot_descriptor_t* descriptor =
      loom_cpu_packed_dot_select(&request, &diagnostic);
  if (descriptor == NULL) {
    return loom_llvmir_lowering_unsupported_op(
        state, op,
        loom_llvmir_lowering_cpu_packed_dot_rejection_detail(
  ...
  if (op->operand_count != 3 || op->result_count != 1) {
    return loom_llvmir_lowering_unsupported_op(
        state, op,
        "CPU packed-dot vector ops require three operands and one "
        "result");
  }
