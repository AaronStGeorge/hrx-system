...
static const char kSourceText[] =
    ...
    "\n"
    "kernel.def target(@target) @double_i32_at_byte_offset(%input: buffer, "
    "%output: buffer, %byte_offset: offset) {\n"
...
static loomc_status_t deserialize_source(emit_spirv_vulkaninfo_state_t* state) {
  loomc_status_t status = loomc_module_deserialize_from_source(
      state->context, state->module_source, NULL, loomc_allocator_system(),
