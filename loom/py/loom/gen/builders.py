            ...
            match element:
                ...
                case DescriptorRef(key=name, stable_id=stable_id):
                    ...
                    if attr_def is None or attr_def.attr_type != "string":
                        raise ValueError(f"Op '{op.name}': DescriptorRef key field '{name}' must be a string attr")
                    if stable_id_attr is None or stable_id_attr.attr_type != "i64":
                        raise ValueError(f"Op '{op.name}': DescriptorRef stable ID field '{stable_id}' must be an i64 attr")
                    params.append(
                        {
                            "name": name,
                            "kind": "descriptor_ref",
                            ...
                            "attr_def": attr_def,
                            "stable_id": stable_id,
                            "doc": attr_def.doc if attr_def else f"Descriptor reference: {name}",
                        }
                    )
                    covered_attrs.add(name)
...
def generate_builders(ops: Sequence[Op], class_name: str) -> str:
    ...
    needs_stable_id = "stable_id_from_string" in body_text
    ...
    if needs_stable_id:
        lines.append("from loom.stable_id import stable_id_from_string")
    lines.append("")
    lines.append("")
...
def _generate_method(op: Op, method_name: str) -> list[str]:
    ...
    for param in params:
        ...
        match param["kind"]:
            ...
            case "descriptor_ref":
                lines.append(f'        _attributes["{param["name"]}"] = {param["name"]}')
                lines.append(f'        _attributes["{param["stable_id"]}"] = stable_id_from_string({param["name"]})')
...
def main() -> None:
    ...
    dialects = [
        ...
        ("check", list(ALL_CHECK_OPS), "CheckBuilders", "check/builders.py"),
