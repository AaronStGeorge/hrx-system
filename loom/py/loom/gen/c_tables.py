...
Usage:
    cd runtime/py && PYTHONPATH=. python3 -m loom.gen.c_tables
"""
...
def generate_ops_h(dialect_name: str, dialect_id: int, ops: Sequence[Op]) -> str:
    ...
    lines.append("// Regenerate: cd runtime/py && PYTHONPATH=. python3 -m loom.gen.c_tables")
    ...
    for op in ops:
        ...
        # Fold function declaration (hand-written, linked in).
        if op.fold:
            lines.append(f"void {op.fold}(")
...
def main() -> None:
    ...
    # loom/py/loom/gen/c_tables.py -> loom/src/loom/ops/
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_root = os.path.normpath(os.path.join(script_dir, "..", "..", "..", "src", "loom", "ops"))

    for dialect, ops in dialects:
        dialect_dir = os.path.join(output_root, dialect.name)
        ...
        ops_h_path = os.path.join(dialect_dir, "ops.h")
        tables_c_path = os.path.join(dialect_dir, "tables.c")
        builders_c_path = os.path.join(dialect_dir, "builders.c")
        ...
        for path, content in [
            ...
            with open(path, "w") as f:
                f.write(content)
        rel = os.path.relpath(dialect_dir, output_root)
