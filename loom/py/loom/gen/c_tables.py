...
def _load_dialect_generation(
    ...
    if loader is None:
        expected = ", ".join(name for name, _ in _DIALECT_GENERATION_LOADERS)
        parser.error(f"unknown dialect {dialect_name!r}; expected one of {expected}")
    return loader()
...
def _main_build_output_mode(parser: argparse.ArgumentParser, args: argparse.Namespace) -> int:
    if args.dialect:
        generation = _load_dialect_generation(parser, args.dialect)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.check:
        return _check_checked_in_outputs(_load_generation_model())
    if args.in_place:
        return _update_checked_in_outputs(_load_generation_model())
