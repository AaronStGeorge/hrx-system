...
class TestParseDynamicDims:
    ...
    def test_ordinal_dim(self) -> None:
        """[#N] in types produces negative dim binding."""
        result, bindings = _parse("tensor<[#1]xf32>")
        assert isinstance(result, ShapedType)
        assert result.dims == (DynamicDim(),)
        assert bindings[0] == -2  # #1 -> -2

    def test_ordinal_dim_zero(self) -> None:
        """[#0] produces -1 in dim bindings."""
        result, bindings = _parse("tensor<[#0]xf32>")
...
class TestParsePoolType:
    ...
    def test_pool_roundtrip(self) -> None:
        ...
        text = (
            "func.def @use_pool(%pool: pool<[%BS]>) -> (pool<[%BS]>) {\n"
...
class TestParsePredicates:
    ...
    def test_single_mul_predicate(self) -> None:
        ...
        module = self._parse_module(
            "func.decl @f(%a: tensor<[%M]xf32>) -> (tensor<[%M]xf32>)"
            " where [mul(%M, 16)]\n"
        )
        op = module.symbols[0].op
        assert op is not None
    ...
    def test_multiple_predicates(self) -> None:
        ...
        module = self._parse_module(
            "func.decl @f(%a: tensor<[%M]x[%K]xf32>) -> (tensor<[%M]xf32>)"
    ...
    def test_pow2_single_arg(self) -> None:
        ...
        module = self._parse_module(
            "func.decl @f(%a: tensor<[%N]xf32>) -> (tensor<[%N]xf32>)"
            " where [pow2(%N)]\n"
        )
        op = module.symbols[0].op
        assert op is not None
    ...
    def test_ordinal_arg(self) -> None:
        """Parse eq(#1, %M) — result ordinal argument."""

        module = self._parse_module(
            "func.decl @f(%a: tensor<[%M]xf32>) -> (tensor<[%M]xf32>, index)"
            " where [eq(#1, %M)]\n"
        )
        op = module.symbols[0].op
        assert op is not None
        ...
        pred = predicates[0]
        assert pred.kind == "eq"
        assert pred.args[0].tag == "ordinal"
        assert pred.args[0].value == 1
    ...
    def test_predicates_with_body(self) -> None:
        ...
        module = self._parse_module(
            "func.def @f(%a: tensor<[%M]xf32>) -> (tensor<[%M]xf32>)"
            " where [mul(%M, 16)] {\n"
...
class TestPredicateRoundTrip:
    ...
    def test_public_device_with_predicates(self) -> None:
        self._roundtrip_text(
            "func.decl public device @vnni(%w: tensor<[%M]x[%K]xi8>) -> (tensor<[%M]xf32>)"
...
class TestParseImportDeclaration:
    ...
    def test_import_with_types(self) -> None:
        ...
        module = self._parse_module(
            'func.decl import("kernels") @conv(%w: tensor<3x3xf32>, %x: tensor<[%N]xf32>)'
            " -> (tensor<[%N]xf32>)\n"
        )
        sym = module.symbols[0]
        assert sym.is_import
        op = sym.op
        assert op is not None
        # func.decl: args as operands, results as result value IDs.
        assert len(op.operands) == 2
        assert len(op.results) == 1
...
class TestDynamicDimScoping:
    ...
    def test_dim_shared_across_args(self) -> None:
        ...
        self._roundtrip_text(
            "func.decl @shared(%a: tile<[%M]xf32>, %b: tile<[%M]xi8>)"
            " -> (tile<[%M]xf32>)\n"
        )

    def test_dim_implicitly_defined_by_first_type(self) -> None:
        """First occurrence of %M in a type defines it (no explicit index arg)."""
        self._roundtrip_text(
            "func.decl @implicit(%a: tile<[%M]xf32>, %b: tile<[%M]xi8>)"
            " -> (tile<[%M]xf32>)\n"
        )
