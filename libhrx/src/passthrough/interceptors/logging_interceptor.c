...
static hipError_t wrap_hipGetDevice(int* deviceId) {
  ...
  log_msg(2, "hipGetDevice() -> device=%d, err=%d", deviceId ? *deviceId : -1,
          err);
  return err;
}
...
static hipError_t wrap_hipGetDeviceCount(int* count) {
  ...
  log_msg(2, "hipGetDeviceCount() -> count=%d, err=%d", count ? *count : -1,
          err);
  return err;
}
...
static hipError_t wrap_hipMemcpy(void* dst, const void* src, size_t sizeBytes,
                                 hipMemcpyKind kind) {
  log_msg(2, "hipMemcpy(dst=%p, src=%p, size=%zu, kind=%s)", dst, src,
          sizeBytes, memcpy_kind_name(kind));
...
static hipError_t wrap_hipModuleLaunchKernel(
  ...
  log_msg(2,
          ...
          blockDimZ, sharedMemBytes, (void*)stream);
