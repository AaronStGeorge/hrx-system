...
"""
            ...
            for j in range(min(3, len(result_lines) - i)):
                ...
                if return_match:
                    ...
                    if default_val.startswith('"'):
                        final_lines.append(
                            f"{indent}const char* _ret = fn ? fn({args}) : {default_val};"
                        )
                        final_lines.append(
                            f'{indent}pt_log(2, "{func_name}() -> %s", _ret ? _ret : "(null)");'
                        )
                    else:
                        final_lines.append(
                            f"{indent}hipError_t _ret = fn ? fn({args}) : {default_val};"
                        )
                        final_lines.append(
                            f'{indent}pt_log(2, "{func_name}() -> %d", _ret);'
                        )
                ...
                if return_noarg_match:
                    ...
                    if default_val.startswith('"'):
                        final_lines.append(
                            f"{indent}const char* _ret = fn ? fn() : {default_val};"
                        )
                        final_lines.append(
                            f'{indent}pt_log(2, "{func_name}() -> %s", _ret ? _ret : "(null)");'
                        )
                        ...
                        final_lines.append(
                            f"{indent}hipError_t _ret = fn ? fn() : {default_val};"
                        )
                        final_lines.append(
                            f'{indent}pt_log(2, "{func_name}() -> %d", _ret);'
                        )
    ...
    enhancements = {
        ...
        "hipMemFree": ("dptr", 'pt_log(2, "hipMemFree(ptr=%p) -> %d", dptr, _ret);'),
...
if __name__ == "__main__":
    ...
    output_path = (
        "/home/awoloszyn/Development/iree-stream/passthrough/hip_intercept.c.new"
    )
