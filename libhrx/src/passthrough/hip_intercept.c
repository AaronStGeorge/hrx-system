...
static void intercept_init(void) {
  ...
  if (interceptor_path && *interceptor_path) {
    ...
    if (g_interceptor_lib) {
      ...
      g_interceptor_shutdown = (pfn_hip_interceptor_shutdown)dlsym(
          g_interceptor_lib, "hip_interceptor_shutdown");

      // Try to get the interceptor's log function for pass-through logging
      pfn_hip_interceptor_get_log_fn get_log_fn =
          (pfn_hip_interceptor_get_log_fn)dlsym(g_interceptor_lib,
                                                "hip_interceptor_get_log_fn");
      if (get_log_fn) {
...
static void ensure_init(void) { pthread_once(&g_init_once, intercept_init); }
...
__attribute__((destructor)) static void intercept_fini(void) {
...
#define FWD0(ret, name)                                            \
  ret name(void) {                                                 \
    ensure_init();                                                 \
    return g_active_table->name ? g_active_table->name() : (ret)0; \
  }

#define FWD1(ret, name, t1, a1)                                      \
  ret name(t1 a1) {                                                  \
    ensure_init();                                                   \
    return g_active_table->name ? g_active_table->name(a1) : (ret)0; \
  }

#define FWD2(ret, name, t1, a1, t2, a2)                                  \
  ret name(t1 a1, t2 a2) {                                               \
    ensure_init();                                                       \
    return g_active_table->name ? g_active_table->name(a1, a2) : (ret)0; \
  }

#define FWD3(ret, name, t1, a1, t2, a2, t3, a3)                              \
  ret name(t1 a1, t2 a2, t3 a3) {                                            \
    ensure_init();                                                           \
    return g_active_table->name ? g_active_table->name(a1, a2, a3) : (ret)0; \
  }

#define FWD4(ret, name, t1, a1, t2, a2, t3, a3, t4, a4)                \
  ret name(t1 a1, t2 a2, t3 a3, t4 a4) {                               \
    ensure_init();                                                     \
    return g_active_table->name ? g_active_table->name(a1, a2, a3, a4) \
                                : (ret)0;                              \
  }
...
#define FWD5(ret, name, t1, a1, t2, a2, t3, a3, t4, a4, t5, a5)            \
  ret name(t1 a1, t2 a2, t3 a3, t4 a4, t5 a5) {                            \
    ensure_init();                                                         \
    return g_active_table->name ? g_active_table->name(a1, a2, a3, a4, a5) \
                                : (ret)0;                                  \
  }
...
hipError_t hipMemcpy2D(void* dst, size_t dpitch, const void* src, size_t spitch,
  ...
  pt_log(2, "hipMemcpy2D(dst=%p, src=%p, width=%zu, height=%zu, kind=%d) -> %d",
         dst, src, width, height, kind, _ret);
  return _ret;
}
...
hipError_t hipMemcpy2DAsync(void* dst, size_t dpitch, const void* src,
  ...
  hipError_t _ret =
      fn ? fn(dst, dpitch, src, spitch, width, height, kind, stream) : 1;
  pt_log(2,
...
hipError_t hipMemcpyDtoD(hipDeviceptr_t dst, hipDeviceptr_t src,
  ...
  pt_log(2, "hipMemcpyDtoD(dst=%p, src=%p, size=%zu) -> %d", dst, src,
         sizeBytes, _ret);
  return _ret;
}
...
hipError_t hipMemcpyDtoH(void* dst, hipDeviceptr_t src, size_t sizeBytes) {
  ...
  hipError_t _ret = fn ? fn(dst, src, sizeBytes) : 1;
  pt_log(2, "hipMemcpyDtoH(dst=%p, src=%p, size=%zu) -> %d", dst, src,
         sizeBytes, _ret);
  return _ret;
}
...
hipError_t hipMemcpyHtoD(hipDeviceptr_t dst, const void* src,
  ...
  hipError_t _ret = fn ? fn(dst, src, sizeBytes) : 1;
  pt_log(2, "hipMemcpyHtoD(dst=%p, src=%p, size=%zu) -> %d", dst, src,
         sizeBytes, _ret);
  return _ret;
}
...
hipError_t hipMemset2D(void* dst, size_t pitch, int value, size_t width,
  ...
  pt_log(2, "hipMemset2D(dst=%p, value=0x%02x, width=%zu, height=%zu) -> %d",
         dst, value, width, height, _ret);
  return _ret;
}
...
hipError_t hipMemsetD8(hipDeviceptr_t dst, unsigned char value, size_t count) {
  ...
  pt_log(2, "hipMemsetD8(dst=%p, value=0x%02x, count=%zu) -> %d", dst, value,
         count, _ret);
  return _ret;
}

hipError_t hipMemsetD8Async(hipDeviceptr_t dst, unsigned char value,
...
hipError_t hipMemsetD16(hipDeviceptr_t dst, unsigned short value,
                        size_t count) {
  ensure_init();
  ...
  hipError_t _ret = fn ? fn(dst, value, count) : 1;
  pt_log(2, "hipMemsetD16(dst=%p, value=0x%04x, count=%zu) -> %d", dst, value,
         count, _ret);
  return _ret;
}

hipError_t hipMemsetD16Async(hipDeviceptr_t dst, unsigned short value,
                             size_t count, hipStream_t stream) {
  ensure_init();
  typedef hipError_t (*pfn)(hipDeviceptr_t, unsigned short, size_t,
                            hipStream_t);
...
hipError_t hipMemsetD32(hipDeviceptr_t dst, int value, size_t count) {
  ...
  hipError_t _ret = fn ? fn(dst, value, count) : 1;
  pt_log(2, "hipMemsetD32(dst=%p, value=0x%08x, count=%zu) -> %d", dst, value,
         count, _ret);
  return _ret;
}
...
hipError_t hipOccupancyMaxActiveBlocksPerMultiprocessor(
  ...
  pt_log(2,
         "hipOccupancyMaxActiveBlocksPerMultiprocessor(func=%p, blockSize=%d, "
         "dynMem=%zu) -> blocks=%d, ret=%d",
         f, blockSize, dynamicSMemSize, numBlocks ? *numBlocks : -1, _ret);
  return _ret;
}
...
hipError_t hipOccupancyMaxActiveBlocksPerMultiprocessorWithFlags(
  ...
  hipError_t _ret =
      fn ? fn(numBlocks, f, blockSize, dynamicSMemSize, flags) : 1;
  pt_log(2, "hipOccupancyMaxActiveBlocksPerMultiprocessorWithFlags() -> %d",
         _ret);
  return _ret;
}
...
hipError_t hipOccupancyMaxPotentialBlockSize(int* gridSize, int* blockSize,
  ...
  hipError_t _ret =
      fn ? fn(gridSize, blockSize, f, dynamicSMemSize, blockSizeLimit) : 1;
  pt_log(
      2,
      "hipOccupancyMaxPotentialBlockSize(func=%p) -> grid=%d, block=%d, ret=%d",
      f, gridSize ? *gridSize : -1, blockSize ? *blockSize : -1, _ret);
  return _ret;
}
...
hipError_t hipDeviceGetPCIBusId(char* pciBusId, int len, int device) {
  ...
  pt_log(2, "hipDeviceGetPCIBusId(device=%d) -> id=%s, ret=%d", device,
         pciBusId ? pciBusId : "(null)", _ret);
  return _ret;
}
...
hipError_t hipIpcOpenMemHandle(void** devPtr, hipIpcMemHandle_t handle,
  ...
  pt_log(2, "hipIpcOpenMemHandle(flags=0x%x) -> ptr=%p, ret=%d", flags,
         devPtr ? *devPtr : NULL, _ret);
  return _ret;
}
...
hipError_t hipGraphCreate(hipGraph_t* pGraph, unsigned int flags) {
  ...
  pt_log(2, "hipGraphCreate(flags=0x%x) -> graph=%p, ret=%d", flags,
         pGraph ? *pGraph : NULL, _ret);
  return _ret;
}
...
hipError_t hipGraphInstantiate(hipGraphExec_t* pGraphExec, hipGraph_t graph,
  ...
  hipError_t _ret =
      fn ? fn(pGraphExec, graph, pErrorNode, pLogBuffer, bufferSize) : 1;
...
hipError_t hipHostRegister(void* hostPtr, size_t sizeBytes,
  ...
  pt_log(2, "hipHostRegister(ptr=%p, size=%zu, flags=0x%x) -> %d", hostPtr,
         sizeBytes, flags, _ret);
  return _ret;
}
...
hipError_t hipHostGetDevicePointer(void** devPtr, void* hostPtr,
  ...
  pt_log(2, "hipHostGetDevicePointer(hostPtr=%p) -> devPtr=%p, ret=%d", hostPtr,
         devPtr ? *devPtr : NULL, _ret);
  return _ret;
}
...
hipError_t hipModuleLoadDataEx(hipModule_t* module, const void* image,
  ...
  hipError_t _ret =
      fn ? fn(module, image, numOptions, options, optionValues) : 1;
  pt_log(2, "hipModuleLoadDataEx(image=%p, numOpts=%u) -> module=%p, ret=%d",
         image, numOptions, module ? *module : NULL, _ret);
  return _ret;
}
...
hipError_t hipModuleOccupancyMaxPotentialBlockSize(int* gridSize,
  ...
  hipError_t _ret =
      fn ? fn(gridSize, blockSize, f, dynSharedMemPerBlk, blockSizeLimit) : 1;
  pt_log(2,
...
hipError_t hipMallocPitch(void** ptr, size_t* pitch, size_t width,
  ...
  pt_log(2,
         "hipMallocPitch(width=%zu, height=%zu) -> ptr=%p, pitch=%zu, ret=%d",
         width, height, ptr ? *ptr : NULL, pitch ? *pitch : 0, _ret);
  return _ret;
}
...
hipError_t hipMallocManaged(void** dev_ptr, size_t size, unsigned int flags) {
  ...
  pt_log(2, "hipMallocManaged(size=%zu, flags=0x%x) -> ptr=%p, ret=%d", size,
         flags, dev_ptr ? *dev_ptr : NULL, _ret);
  return _ret;
}
...
hipError_t hipMemAdvise(const void* dev_ptr, size_t count, int advice,
  ...
  pt_log(2, "hipMemAdvise(ptr=%p, count=%zu, advice=%d, device=%d) -> %d",
         dev_ptr, count, advice, device, _ret);
  return _ret;
}
...
hipError_t hipCtxCreate(hipCtx_t* ctx, unsigned int flags, hipDevice_t device) {
  ...
  pt_log(2, "hipCtxCreate(flags=0x%x, device=%p) -> ctx=%p, ret=%d", flags,
         device, ctx ? *ctx : NULL, _ret);
  return _ret;
}
...
hipError_t hipDeviceComputeCapability(int* major, int* minor,
  ...
  pt_log(2,
         "hipDeviceComputeCapability(device=%p) -> major=%d, minor=%d, ret=%d",
         device, major ? *major : -1, minor ? *minor : -1, _ret);
  return _ret;
}
...
hipError_t hipDeviceTotalMem(size_t* bytes, hipDevice_t device) {
  ...
  pt_log(2, "hipDeviceTotalMem(device=%p) -> bytes=%zu, ret=%d", device,
         bytes ? *bytes : 0, _ret);
  return _ret;
}
...
hipError_t hipMemAllocPitch(hipDeviceptr_t* dptr, size_t* pitch,
  ...
  hipError_t _ret =
      fn ? fn(dptr, pitch, widthInBytes, height, elementSizeBytes) : 1;
...
hipError_t hipMemAlloc(hipDeviceptr_t* dptr, size_t size) {
  ...
  pt_log(2, "hipMemAlloc(size=%zu) -> ptr=%p, ret=%d", size,
         dptr ? *dptr : NULL, _ret);
  return _ret;
}
...
hipError_t hipMemcpyPeer(void* dst, int dstDeviceId, const void* src,
  ...
  pt_log(2,
         "hipMemcpyPeer(dst=%p, dstDev=%d, src=%p, srcDev=%d, size=%zu) -> %d",
         dst, dstDeviceId, src, srcDeviceId, sizeBytes, _ret);
  return _ret;
}
...
hipError_t hipMemcpyPeerAsync(void* dst, int dstDeviceId, const void* src,
  ...
  hipError_t _ret =
      fn ? fn(dst, dstDeviceId, src, srcDeviceId, sizeBytes, stream) : 1;
...
hipError_t hipExtLaunchKernel(const void* function_address, dim3 numBlocks,
  ...
  hipError_t _ret =
      fn ? fn(function_address, numBlocks, dimBlocks, args, sharedMemBytes,
...
hipError_t hipDeviceGetLimit(size_t* pValue, int limit) {
  ...
  pt_log(2, "hipDeviceGetLimit(limit=%d) -> value=%zu, ret=%d", limit,
         pValue ? *pValue : 0, _ret);
  return _ret;
}
...
hipError_t hipGetDeviceFlags(unsigned int* flags) {
  ...
  hipError_t _ret = fn ? fn(flags) : 1;
  pt_log(2, "hipGetDeviceFlags() -> flags=0x%x, ret=%d", flags ? *flags : 0,
         _ret);
  return _ret;
}
...
hipError_t hipGraphAddMemcpyNode(hipGraphNode_t* pGraphNode, hipGraph_t graph,
  ...
  hipError_t _ret =
      fn ? fn(pGraphNode, graph, pDependencies, numDependencies, pCopyParams)
         : 1;
...
hipError_t hipGraphAddMemsetNode(hipGraphNode_t* pGraphNode, hipGraph_t graph,
  ...
  hipError_t _ret =
      fn ? fn(pGraphNode, graph, pDependencies, numDependencies, pMemsetParams)
         : 1;
...
hipError_t hipGraphAddKernelNode(hipGraphNode_t* pGraphNode, hipGraph_t graph,
  ...
  hipError_t _ret =
      fn ? fn(pGraphNode, graph, pDependencies, numDependencies, pNodeParams)
         : 1;
...
hipError_t hipLaunchCooperativeKernel(const void* f, dim3 gridDim,
  ...
  hipError_t _ret =
      fn ? fn(f, gridDim, blockDim, kernelParams, sharedMemBytes, stream) : 1;
  pt_log(2,
...
hipError_t hipOccupancyMaxPotentialBlockSizeWithFlags(
  ...
  if (!fn)
    fn =
        (pfn)dlsym(g_backend_lib, "hipOccupancyMaxPotentialBlockSizeWithFlags");
  hipError_t _ret =
      fn ? fn(gridSize, blockSize, f, dynamicSMemSize, blockSizeLimit, flags)
         : 1;
...
hipError_t hipMemcpy2DToArray(hipArray_t dst, size_t wOffset, size_t hOffset,
  ...
  hipError_t _ret =
      fn ? fn(dst, wOffset, hOffset, src, spitch, width, height, kind) : 1;
...
hipError_t hipMemcpy2DFromArray(void* dst, size_t dpitch, hipArray_t src,
  ...
  hipError_t _ret =
      fn ? fn(dst, dpitch, src, wOffset, hOffset, width, height, kind) : 1;
...
hipError_t hipExtMallocWithFlags(void** ptr, size_t sizeBytes,
  ...
  pt_log(2, "hipExtMallocWithFlags(size=%zu, flags=0x%x) -> ptr=%p, ret=%d",
         sizeBytes, flags, ptr ? *ptr : NULL, _ret);
  return _ret;
}
...
void __hipRegisterTexture(void** fatCubinHandle, char* hostVar,
  ...
  if (fn)
    fn(fatCubinHandle, hostVar, deviceAddress, deviceName, dim, norm, ext);
...
hipError_t hipUserObjectCreate(void** object_out, void* ptr,
  ...
  hipError_t _ret =
      fn ? fn(object_out, ptr, destroy, initialRefcount, flags) : 1;
...
hipError_t hipGraphAddEventRecordNode(hipGraphNode_t* pGraphNode,
  ...
  hipError_t _ret =
      fn ? fn(pGraphNode, graph, pDependencies, numDependencies, event) : 1;
...
hipError_t hipGraphAddChildGraphNode(hipGraphNode_t* pGraphNode,
  ...
  hipError_t _ret =
      fn ? fn(pGraphNode, graph, pDependencies, numDependencies, childGraph)
         : 1;
...
hipError_t hipGraphAddEmptyNode(hipGraphNode_t* pGraphNode, hipGraph_t graph,
  ...
  hipError_t _ret =
      fn ? fn(pGraphNode, graph, pDependencies, numDependencies) : 1;
...
hipError_t hipGraphExecUpdate(hipGraphExec_t hGraphExec, hipGraph_t hGraph,
  ...
  hipError_t _ret =
      fn ? fn(hGraphExec, hGraph, hErrorNode_out, updateResult_out) : 1;
...
hipError_t hipGraphAddMemFreeNode(hipGraphNode_t* pGraphNode, hipGraph_t graph,
  ...
  hipError_t _ret =
      fn ? fn(pGraphNode, graph, pDependencies, numDependencies, dptr) : 1;
...
hipError_t hipLaunchKernelGGL(const void* func, dim3 gridDim, dim3 blockDim,
  ...
  hipError_t _ret =
      fn ? fn(func, gridDim, blockDim, sharedMem, stream, kernelParams) : 1;
...
hipError_t hipModuleOccupancyMaxPotentialBlockSizeWithFlags(
  ...
  hipError_t _ret =
      fn ? fn(gridSize, blockSize, f, dynSharedMemPerBlk, blockSizeLimit, flags)
         : 1;
...
hipError_t hipModuleOccupancyMaxActiveBlocksPerMultiprocessorWithFlags(
  ...
  hipError_t _ret =
      fn ? fn(numBlocks, f, blockSize, dynSharedMemPerBlk, flags) : 1;
  pt_log(2,
         "hipModuleOccupancyMaxActiveBlocksPerMultiprocessorWithFlags() -> %d",
         _ret);
  return _ret;
}
...
hipError_t hipHccModuleLaunchKernel(
  ...
  hipError_t _ret =
      fn ? fn(f, globalWorkSizeX, globalWorkSizeY, globalWorkSizeZ, blockDimX,
...
hipError_t hipModuleLaunchCooperativeKernel(
  ...
  hipError_t _ret =
      fn ? fn(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ,
...
hipError_t hipOccupancyMaxPotentialBlockSizeVariableSMem(
  ...
  hipError_t _ret = fn ? fn(minGridSize, blockSize, func,
...
hipError_t hipOccupancyMaxPotentialBlockSizeVariableSMemWithFlags(
  ...
  hipError_t _ret = fn ? fn(minGridSize, blockSize, func,
                       ...
                       : 1;
  pt_log(2, "hipOccupancyMaxPotentialBlockSizeVariableSMemWithFlags() -> %d",
         _ret);
  return _ret;
}
...
hipError_t hipRtcCreateProgram(void** prog, const char* src, const char* name,
  ...
  hipError_t _ret =
      fn ? fn(prog, src, name, numHeaders, headers, includeNames) : 1;
...
hipError_t hipGLGetDevices(unsigned int* pHipDeviceCount, int* pHipDevices,
  ...
  hipError_t _ret =
      fn ? fn(pHipDeviceCount, pHipDevices, hipDeviceCount, deviceList) : 1;
...
hipError_t hipLaunchKernel(const void* function_address, dim3 numBlocks,
  ...
  return g_active_table->hipLaunchKernel
             ? g_active_table->hipLaunchKernel(function_address, numBlocks,
...
hipError_t hipExtModuleLaunchKernel(
  ...
  return g_active_table->hipExtModuleLaunchKernel
             ? g_active_table->hipExtModuleLaunchKernel(
                   f, globalWorkSizeX, globalWorkSizeY, globalWorkSizeZ,
                   localWorkSizeX, localWorkSizeY, localWorkSizeZ,
                   sharedMemBytes, stream, kernelParams, extra, startEvent,
                   stopEvent, flags)
             : 1;
}
